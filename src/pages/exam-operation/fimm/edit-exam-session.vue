...
export default {
  mounted () {
    this.editDetails = JSON.parse(localStorage.getItem('EDIT_EXAM_SESSION'))
  ...
  methods: {
    ...
    validateLink (value) {
      if (value) {
        const Reg = /^http\:\/\/|https\:\/\/|teams.microsoft.com$/
        if (!value.match(Reg)) {
    ...
    submit: async function () {
      ...
      if (response) {
        if (confirm('Are you sure you want to edit this session?')) {
          ...
          try {
            await examSessionManagementServices.editExamSession(data)
            this.back()
          } catch (error) {
            console.log(error)
          }
    ...
    return {
      ...
      editSessionSchema: {
        fields: [
          ...
          // {
          //   type: 'radios',
          //   label: 'Exam Waiver Status',
          //   model: 'examWaiverStatus',
