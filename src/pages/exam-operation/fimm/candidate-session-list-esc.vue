  ...
  <va-card>
    ...
    <va-card class="mt-5 col-12">
      ...
      >
        ...
        <template slot="attendance" slot-scope="props" >
          <h5 class="mt-1">
            <select style="width:90px;padding:0px"
              :csl="[props.rowData.ID]"
              class="form-control final-attendance">
              <template v-if="props.rowData.ATTENDANCE === 'PRESENT'">
                <option value="1" selected>PRESENT</option>
                <option value="0">ABSENT</option>
              </template>
              <template v-if="props.rowData.ATTENDANCE === 'ABSENT'">
                <option value="1" >PRESENT</option>
                <option value="0" selected>ABSENT</option>
              </template>
              <option hidden>{{props.rowData.ID}}</option>
            </select>
          </h5>
        </template>
      </va-data-table>
      ...
      <div class="float-right">
        ...
        >
          <i class="fa fa-paper-plane"/> &nbsp; Save Attendance
        </button>
      </div>
    </va-card>
  </va-card>
  ...
  methods: {
    ...
    submit: async function () {
      if (confirm('Are you sure you want to proceed this submission?')) {
        ...
        data.append('NEW_DATA', JSON.stringify(this.getDatas()))
        try {
          await examSessionManagementServices.saveCandidateAttendance(data)
          // this.back()
        } catch (error) {
          console.log(error)
        }
    ...
    getDatas () {
      const data = []
      const invigilInputs = document.querySelectorAll('select.final-attendance')
      for (let index = 0; index < invigilInputs.length; index++) {
        const cslId = invigilInputs[index].getAttribute('csl')
        if (invigilInputs[index].value.length) {
          data.push(
            { cslId: cslId, value: Number(invigilInputs[index].value) },
          )
        }
      }
      return data
