<template>
  <va-inner-loading :loading="statusLoad">
    <va-card>
      <div class="row">
        <h4 class="card-title" slot="header">List of Company</h4>
        <br />
        ...
        >
          ...
          <template slot="EXAM_CENTRE_LIST" slot-scope="props">
            <button
              ...
              v-on:click="viewExamCentreList(props.rowData)"
            >
              EXAM CENTRE
            </button>
          </template>
...
export default {
  ...
  methods: {
    ...
    getAllRecord: async function () {
      this.RecordList = await companyManagementServices.listAllCompany()
    ...
    async editRecord (data) {
      localStorage.setItem('edit-company-profile-id', data.COMPANY_PROFILE_ID)
      this.$router.push({
        name: 'edit-company-profile',
      })
    },
    async viewExamCentreList (data) {
      localStorage.setItem('view-exam-centre-list', JSON.stringify({
    ...
    async filterCompanyRecord () {
      ...
      try {
        const response = await companyManagementServices.listAllCompany({ FILTERS: this.model })
        if (response !== 'error') {
