...
export default {
  ...
  mounted () {
    const data = JSON.parse(localStorage.getItem('review-company-profile'))
  ...
  methods: {
    ...
    review: async function () {
      ...
      if (response && confirm('Are you sure you want to review this submission?')) {
        ...
        try {
          await companyManagementServices.reviewCompanyProfile(data)
          this.back()
        } catch (error) {
          console.log(error)
        }
    ...
    reject: async function () {
      ...
      if (response && confirm('Are you sure you want to reject this submission?')) {
        ...
        const user = localStorage.getItem('user')
        data.append('CREATE_BY', JSON.parse(user).user_id)
        data.append('REJECT_BY', 'PDS-REVIEWER')
