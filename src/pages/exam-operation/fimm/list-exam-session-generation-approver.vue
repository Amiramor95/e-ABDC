  ...
  <va-inner-loading :loading="statusLoad">
    <va-card>
      ...
      <va-card class="mt-5 col-12">
        ...
        <br />
        <div class="approver-section" v-if="isPendingStatus()">
          <div>
...
export default {
  ...
  methods: {
    ...
    isPendingStatus () {
      return localStorage.getItem('APPROVAL_STATUS') === TaskStatusConstants.PENDING_DISPLAY
    },
    ...
    returnSession: async function () {
      if (this.RecordCount === 0) {
        ...
        if (confirm('Are you sure you want to return the submission?')) {
          ...
          data.append('NOTI_REMARK', '(EB) Exam Session Returned by Approver.')
    ...
    submit: async function () {
      if (this.RecordCount === 0) {
        ...
        if (confirm('Are you sure you want to approve the submission?')) {
          const data = new FormData()
          data.append('EXAM_SESSION_APPROVAL_ID', localStorage.getItem('EXAM_SESSION_APPROVAL_ID'))
          data.append('PUBLISH_STATUS', TaskStatusConstants.PUBLISH_STATUS)
          ...
          try {
            await examSessionManagementServices.approveExamSession(data)
            this.$router.go(-1)
          } catch (error) {
            console.log(error)
          }
