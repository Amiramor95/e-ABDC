  ...
  methods: {
    ...
    async submit () {
      ...
      if (response && confirm('Are you sure you want to proceed this submission ?')) {
        ...
        // // Send to all pds user
        // const level = await examBookingServices.getPDSApprovalLevel()
        // data1.append('USER_LIST', JSON.stringify(level))
        // data1.append('NOTI_LOCATION', 'list-COMPANY-management')
        // data1.append('NOTI_REMARK', '(EB) New COMPANY registered.')

        try {
          await companyManagementServices.createCompany(data1)
    ...
    async getDefaultCountry () {
      try {
        ...
        this.model.COMPANY_COUNTRY = response
      } catch (error) {
        // console.log(error);
      }
    },
    ...
    onModelUpdated (newVal, schema) {
      ...
      if (schema === 'COMPANY_ADMIN_CITIZENSHIP') {
        ...
        if (this.model.COMPANY_COUNTRY.SET_CODE === 'MY') {
          this.getCity(this.model.COMPANY_POSTAL.SETTING_CITY_ID)
        }
