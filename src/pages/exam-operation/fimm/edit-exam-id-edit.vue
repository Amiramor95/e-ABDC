...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            this.editData = JSON.parse(localStorage.getItem('edit-exam-id'));
            this.loadData();
            this.getAllExamType();
        },
        data() {
                    ...
                    groups: [
                        {
                            ...
                            fields: [
                                ...
                                },
                                {
                                    ...
                                    required: true,
                                    validator: ["required", this.validateGapDuration]
                                },
                                {
                                    labels: "END TIME",
                                    type: "vfg-time-picker",
        ...
        methods: {
            capitalize()
            {
                let elems = document.querySelectorAll(".form-group label span");
            ...
            async loadData(){
                    this.model.day = this.editData.DAY;
                    this.model.startTime = this.editData.FLAT_START_TIME;
                    this.model.endTime = this.editData.FLAT_END_TIME;
                    this.model.english = this.editData.EXAM_ID_ENGLISH ?? "";
                    this.model.malay = this.editData.EXAM_ID_MALAY ?? "";
                    this.model.chinese = this.editData.EXAM_ID_CHINESE ?? "";
                    this.examId = this.editData.ID;
            },
            async getAllExamType() {
                this.examType = await examBookingServices.getActiveExamTypesDetails();
                this.model.examType = this.examType.find(x => x.ID === this.editData.EXAM_TYPE_DETAILS_ID);
            },
            hasZeroTime(m) {
            ...
            validateGapDuration()
            {
                ...
                if(this.duration !== undefined) {
                    // let startTime = moment(this.model.startTime, 'HH:mm:ss');
                    // let endTime = moment(this.model.endTime, 'HH:mm:ss');
            ...
            validateStartTime(value) {
                if (this.model.startTime != null) {
                    let startTime = moment(moment().format('MM/DD/YYYY')+''+this.model.startTime, 'MM/DD/YYYY HH:mm:ss');
                    let todayTime = moment(moment().format('MM/DD/YYYY HH:mm:ss'), 'MM/DD/YYYY HH:mm:ss');
                    if (startTime.isSameOrAfter(todayTime)) {
                        console.log("Valid");
                    } else {
                ...
                if (this.model.endTime != null) {
                    let startTime = moment(moment().format('MM/DD/YYYY')+''+this.model.startTime, 'MM/DD/YYYY HH:mm:ss');
                    let endTime = moment(moment().format('MM/DD/YYYY')+''+this.model.endTime, 'MM/DD/YYYY HH:mm:ss');
            ...
            submit: async function () {
                ...
                if (response) {
                            ...
                            data1.append("EXAM_ID", this.examId);
                            ...
                            try {
                                const response = await centreManagementServices.updateExamId(data1);
                                localStorage.setItem("new-exam-id", JSON.stringify(this.model));
                                this.$router.push({name: "edit-exam-centre"},);
                            } catch (error) {
                                console.log(error);
