  ...
  methods: {
    ...
    async validateUserID (value) {
      ...
      if (idlength >= 1) {
        ...
        const response = await services03Module5.checkDuplicateUserID(value)
        if (response) {
      ...
      if (response) {
        if (this.model.citizenship == '1') {
          if (this.model.registrationType == '1') {
            ...
            data.append('TP_USER_POSTAL', this.model.applicantPostcode)
            data.append('TP_USER_CITY', this.model.applicantCity)
            data.append('TP_USER_STATE', this.model.applicantState)
