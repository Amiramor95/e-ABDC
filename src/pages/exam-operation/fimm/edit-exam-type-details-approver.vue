      ...
      <div class="ml-3 col-md-12">
        <table class="table table-striped">
          <tbody>
            ...
            <tr>
              <td style="width: 15%">LICENSE TYPE</td>
...
export default {
  ...
  mounted () {
    const data = localStorage.getItem('edit-exam-type-details-approver')
  ...
  methods: {
    ...
    approve: async function () {
      if (confirm('Are you sure you want to proceed this submission?')) {
        ...
        data.append('NOTI_REMARK', '(EB) Exam type details Approved.')

        try {
          await examBookingServices.approveExamTypeDetails(data)
          this.back()
        } catch (error) {
          console.log(error)
        }
    ...
    reject: async function () {
      if (confirm('Are you sure you want to reject this submission?')) {
        ...
        const userLevel = await services06Module0.getApprovalLevel(
          [ApprovalLevelConstants.PROFESSIONAL_DEVELOPMENT_USER_PROCESS_ID,
            ApprovalLevelConstants.PROFESSIONAL_DEVELOPMENT_USER_PROCESS_ID],
        )
        data.append('APPR_LIST', JSON.stringify(userLevel))
        data.append('NOTI_LOCATION', 'list-exam-type-details')
        data.append('NOTI_REMARK', '(EB) This Entry has been rejected by Approver')

        // dashboad notification to reviewer
        ...
        data.append('APPR_LIST2', JSON.stringify(reviewerLevel))
        data.append('NOTI_LOCATION2', 'list-exam-type-details-reviewer')
        data.append('NOTI_REMARK2', '(EB) This Entry has been rejected by Approver')
    ...
    returned: async function () {
      if (confirm('Are you sure you want to return this submission?')) {
        ...
        data.append('NOTI_LOCATION', 'list-exam-type-details')
        data.append('NOTI_REMARK', '(EB) This Entry has been returned by Approver')
        try {
