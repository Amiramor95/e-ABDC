          ...
          <div class="float-right">
            <h5>
              ...
              <span
                ...
                v-b-tooltip.hover
                v-on:click="create"
                v-if="(this.taskStatus === taskStatusConstants.NEW_ENTRY_DISPLAY
                  || this.taskStatus === taskStatusConstants.PENDING_DISPLAY)"
              >
          ...
          <template slot="actions" slot-scope="props">
            <h5 class="mt-1">
              <span
                ...
                v-if="(props.rowData.APPROVAL_STATUS === taskStatusConstants.NEW_ENTRY_DISPLAY
                  || props.rowData.APPROVAL_STATUS === taskStatusConstants.PENDING_DISPLAY
                  || props.rowData.APPROVAL_STATUS === taskStatusConstants.REVIEWED_DISPLAY)"
                class="badge badge-primary mr-2">
    ...
    submit: async function () {
      if (this.RecordCount === 0) {
        ...
        if (confirm('Are you sure you want to proceed the submission?')) {
          const data = new FormData()
          data.append('ID', localStorage.getItem('SESSION_ID'))
          data.append('PUBLISH_STATUS', TaskStatusConstants.PUBLISH_STATUS)

          data.append('NOTI_LOCATION', 'exam-session-generation-reviewer')
          data.append('NOTI_REMARK', '(EB) New entry for Exam Session Submitted.')
          ...
          const reviewerLevel = await services06Module0.getApprovalLevel(
            ApprovalLevelConstants.PROFESSIONAL_DEVELOPMENT_REVIEWER_EXAM_SESSION_PROCESS_ID,
            ApprovalLevelConstants.EXAM_TYPE_REVIEW_APPR_INDEX,
          )
          data.append('REVIEWER_LIST', JSON.stringify(reviewerLevel))

          try {
            await examSessionManagementServices.submitExamSession(data)
            this.$router.go(-1)
          } catch (error) {
            console.log(error)
          }
