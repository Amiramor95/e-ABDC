<template>
  <va-card>
    ...
    <div>
      ...
      <div class="mt-5">
        <button @click="$router.push('extension-request')" type="button" class="btn btn-primary btn-fill btn-md">
          <i class="fa fa-step-backward" /> &nbsp; Previous
        </button>
        <div class="float-right">
          <button @click="model.submissionStatus = 'DRAFT'" type="submit" class="btn btn-info btn-fill btn-md mr-2">
            <i class="fa fa-edit" /> &nbsp; Save As Draft
          </button>
          <button @click="model.submissionStatus = 'PENDING'" type="submit" class="btn btn-success btn-fill btn-md mr-2">
...
export default {
  ...
  methods: {
    moment,
    onsubmit: async function(action) {
      ...
      if(data.STATUS === "PENDING") {
        ...
        toastMessage = `${toastMessage} submit Extension Request.`
      ...
      if(pass === true) {
        ...
        this.$router.push('extension-request');
    ...
    onBeforeDelete(fileRecord){
      ...
      if (i !== -1) {
        ...
        this.model.file = this.uploadFileOject;
      } else {
    ...
    deleteDocument: async function (isActionPlan, val) {
     const response = await servicesModule1.deleteUploadedDocument(isActionPlan, val);
    },
    ...
    onBeforeDeleteActionPlan(fileRecord){
      ...
      if (i !== -1) {
        ...
        for (i = 0; i < this.fileActionPlanRecordsForUpload.length; i++) {
          this.uploadFileOject.push(this.fileActionPlanRecordsForUpload[i].file);
        }
        console.log(this.uploadFileOject);
    ...
    deleteUploadedFileActionPlan: function(fileRecord) {
      ...
      var i;
      for (i = 0; i < this.fileActionPlanRecordsForUpload.length; i++) {
        this.uploadFileOjectActionPlan.push(this.fileActionPlanRecordsForUpload[i].file);
      }
    ...
    filesSelectedActionPlan: function(fileRecordsNewlySelected) {
      ...
      this.fileActionPlanRecordsForUpload = this.fileActionPlanRecordsForUpload.concat(
        validFileRecords
      );
