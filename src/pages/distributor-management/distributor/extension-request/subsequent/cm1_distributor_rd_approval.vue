  ...
  <va-card>
    ...
    <va-card class="mt-3">
      ...
      <b-collapse id="collapse-2">
        <div class="row mt-1">
          ...
          <div class="col-md-6">
            Submission Date <br>
            {{subsequentModel.submissionDate !== null ? moment(subsequentModel.submissionDate).format('DD MMM YYYY') : null}}
          </div>
        </div>
        <div>
          Supporting Documents
        ...
        <va-card class="mt-2">
          <div class="row">
            ...
            <div class="col float-right">
              <div class="float-right">
                <h5>
                  <b-badge v-b-toggle.collapse-3 class="mr-1" variant="secondary">
                    <i class="fa fa-eye"></i>
                  </b-badge>
                </h5>
              </div>
    ...
    <va-card class="mt-3">
      ...
      <form @submit.prevent="onApprove" enctype="multipart/form-data">
        ...
        <div class="mt-5">
          <button
            @click="$router.push({name: 'subsequent-extension-request-details'})"
            type="button"
...
export default {
  ...
  methods: {
    ...
    getById: async function () {
      ...
      this.approvalModel.justification = response.MANAGER_APPROVAL.APPROVAL_REMARK;

      this.fileParser(response.MANAGER_APPROVAL.DOCUMENTS, this.approvalModel);
      ...
      if(response.hasOwnProperty('RD_HOD_APPROVAL')) {
        if(response.RD_HOD_APPROVAL !== null) {
          this.returned = true;
          this.ammendmentModel.justification = response.RD_HOD_APPROVAL.APPROVAL_REMARK;

          this.fileParser(response.RD_HOD_APPROVAL.DOCUMENTS, this.ammendmentModel)
        }
      }
