  ...
  <va-card>
    <div class="row">
      <div slot="header" class="card-title">
        <h4>Time Extension Request List</h4>
      </div>
    </div>
    <va-data-table
...
export default {
  ...
  methods: {
    ...
    getData: async function() {
      let res = await servicesModule1.approvalLists();
      this.extensionRequests = res.data;
      this.dataCount = res.data.length;
    },
    redirect(index) {
      localStorage.setItem('RE_ID', this.extensionRequests[index].EXTENSION_REQUEST_ID);
      this.$router.push('view-extension-request');
    }
  },
  created() {
    this.getData();
