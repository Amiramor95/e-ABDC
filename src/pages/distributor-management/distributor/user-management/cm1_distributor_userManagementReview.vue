<template>
  <div class="content">
    <div class="container-fluid">
      <div class="card-body">
        <div class="row justify-content-center">
          <div class="col-lg-12">
            <va-card
              class="striped-tabled-with-hover"
              body-classes="table-full-width table-responsive"
            >
              <h4 slot="header" class="card-title">
                FIMM User Registration Details
              </h4>

              <br />
              <div>
                <tabs
                  :tabs="tabs"
                  :currentTab="currentTab"
                  :wrapper-class="'default-tabs'"
                  :tab-class="'default-tabs__item'"
                  :tab-active-class="'default-tabs__item_active'"
                  :line-class="'default-tabs__active-line'"
                  @onClick="handleClick"
                />
                <div class="content">
                  <div v-if="currentTab === 'tab1'">
                    <div class="ml-4">
                      <br />
                      <table class="table table-striped">
                        <tbody>
                          <tr>
                            <td style="width: 20%">Name</td>
                            <td style="width: 2%">:</td>
                            <td>{{ getDataUser.USER_NAME }}</td>
                          </tr>
                          <tr>
                            <td>NRIC No</td>
                            <td>:</td>
                            <td>{{ getDataUser.USER_NRIC }}</td>
                          </tr>
                          <tr v-if="getDataUser.USER_NRIC == null">
                            <td>Passport No</td>
                            <td>:</td>
                            <td></td>
                          </tr>
                          <tr>
                            <td>Telephone Number</td>
                            <td>:</td>
                            <td>
                              +60{{
                                getDataUser.USER_MOBILE_NUM.substring(0, 1)
                              }}-{{
                                getDataUser.USER_MOBILE_NUM.substring(1, 10)
                              }}
                            </td>
                          </tr>
                          <tr>
                            <td>Email</td>
                            <td>:</td>
                            <td>{{ getDataUser.USER_EMAIL }}</td>
                          </tr>
                          <tr>
                            <td>Address</td>
                            <td>:</td>
                            <td>
                              {{ getDataUser.USER_ADDR_1 }}
                              {{ getDataUser.USER_ADDR_2 }}
                              {{ getDataUser.USER_ADDR_3 }}
                            </td>
                          </tr>
                          <tr>
                            <td>Postcode</td>
                            <td>:</td>
                            <td></td>
                          </tr>
                          <tr>
                            <td>City</td>
                            <td>:</td>
                            <td></td>
                          </tr>
                          <tr>
                            <td>State</td>
                            <td>:</td>
                            <td></td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
                  </div>
                  <div v-if="currentTab === 'tab2'">
                    <div class="ml-4">
                      <br />
                      <table class="table table-striped">
                        <tbody>
                          <tr>
                            <td style="width: 20%">Division</td>
                            <td style="width: 2%">:</td>
                            <td>
                              <!-- {{ getDataUser.DISTRIBUTOR_MANAGE_GROUP_NAME }} -->
                            </td>
                          </tr>
                          <tr>
                            <td>Department</td>
                            <td>:</td>
                            <td>
                              <!-- {{ getDataUser.DISTRIBUTOR_MANAGE_GROUP_NAME }} -->
                            </td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
                  </div>
                  <div v-if="currentTab === 'tab3'">
                    <div class="ml-4">
                      <br />
                      <table class="table table-striped">
                        <tbody>
                          <tr>
                            <td style="width: 20%">Roles</td>
                            <td style="width: 2%">:</td>
                            <td>
                              {{ getDataUser.DISTRIBUTOR_MANAGE_GROUP_NAME }}
                            </td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
                  </div>
                </div>
              </div>
            </va-card>
          </div>
        </div>
      </div>
    </div>
    <div class="container-fluid">
      <div class="card-body">
        <div class="row justify-content-center">
          <div class="col-lg-12">
            <va-card
              class="striped-tabled-with-hover"
              body-classes="table-full-width table-responsive"
            >
              <h4 slot="header" class="card-title">Registration Approval</h4>
              <form
                v-if="
                  getDataUser.TS_PARAM == 'NEW ENTRY' ||
                  getDataUser.TS_PARAM == 'PENDING'
                "
              >
                <vue-form-generator
                  :model="model"
                  :schema="schema"
                  :options="formOptions"
                  @model-updated="onModelUpdated"
                  ref="statusForm"
                >
                </vue-form-generator>

                <button
                  @click="back"
                  type="submit"
                  class="ml-3 btn btn-primary btn-fill float-left btn-md"
                >
                  <i class="fa fa-step-backward" /> &nbsp; Previous
                </button>
                <div
                  class="float-right"
                  v-if="
                    getDataUser.TS_PARAM == 'NEW ENTRY' ||
                    getDataUser.TS_PARAM == 'PENDING'
                  "
                >
                  <button
                    @click.once="returnData"
                    type="button"
                    class="mr-2 btn btn-warning btn-fill btn-md"
                  >
                    <i class="fa fa-reply" /> &nbsp; Return
                  </button>
                  <button
                    @click="onSubmit"
                    type="button"
                    class="btn btn-success btn-fill btn-md"
                  >
                    <i class="fa fa-paper-plane" /> &nbsp; Approve
                  </button>
                </div>
              </form>
            </va-card>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>
<script>
import Vue from "vue";
import "vue-form-generator/dist/vfg.css";
import Tabs from "vue-tabs-with-active-line";
import * as services6Module1 from "../../../../app/module1/services06";
const TABS = [
  {
    title: "User Profile",
    value: "tab1",
  },
  {
    title: "Division & Department",
    value: "tab2",
  },

  {
    title: "Roles",
    value: "tab3",
  },
];
export default {
  components: {
    Tabs,
  },

  data() {
    return {
      tabs: TABS,
      getDataUser: this.$route.params.USERDETAIL,
      currentTab: "tab1",

      //FORM
      model: {
        Remark: "",
      },
      schema: {
        fields: [
          {
            type: "textArea",
            label: "Remark",
            model: "Remark",
            hint: "Max 500 characters",
            max: 500,
            rows: 4,
            required: true,
            validator: "string",
          },
        ],
      },

      formOptions: {
        validateAfterLoad: false,
        validateAfterChanged: true,
        validateAsync: true,
      },
    };
  },

  methods: {
    back() {
      this.$router.push({ name: "distributor-user-management-list" });
    }, // back button
    validatestatusForm() {
      return this.$refs.statusForm.validate();
    },
    returnData: async function () {
      const response = await this.validatestatusForm();
      if (response) {
        if (
          confirm(
            "Are you sure to return this user registration and role selection?"
          )
        ) {
          const user = localStorage.getItem("user");
          const data = new FormData();
          data.append("USER_REGI_APPR_ID", this.getDataUser.USER_REGI_APPR_ID);
          data.append("DIST_ID", JSON.parse(user).USER_DIST_ID);
          data.append("APPR_REMARK", this.model.Remark);
          data.append("APPR_STATUS", 8);
          data.append("APPR_GROUP_ID", JSON.parse(user).USER_GROUP_ID);
          // data.append("APPROVAL_LEVEL_ID",JSON.parse(user).);
          try {
            const response = await services6Module1.updateApproval(data);
            alert("User registration and role selection has been returned.");
            this.$router.push({ name: "distributor-user-management-list" });
          } catch (e) {
            console.log(e);
          }
        } else {
        }
      } else {
      }
    },
    onSubmit: async function () {
      const response = await this.validatestatusForm();
      console.log("start");
      if (response) {
        if (
          confirm(
            "Are you sure to approve this user registration and role selection"
          )
        ) {
          const user = localStorage.getItem("user");
          const data = new FormData();
          data.append("USER_REGI_APPR_ID", this.getDataUser.USER_REGI_APPR_ID);
          data.append("DIST_ID", JSON.parse(user).USER_DIST_ID);
          data.append("APPR_REMARK", this.model.Remark);
          data.append("APPR_STATUS", 3);
          data.append("APPR_GROUP_ID", JSON.parse(user).USER_GROUP_ID);
          data.append(
            "DISTRIBUTOR_MANAGE_GROUP_ID",
            this.getDataUser.DISTRIBUTOR_MANAGE_GROUP_ID
          );
          // data.append("APPROVAL_LEVEL_ID");
          try {
            const response = await services6Module1.updateApproval(data);
            alert(JSON.stringify(response));
            alert("User registration and role selection has been approved.");
            this.$router.push({ name: "distributor-user-management-list" });
          } catch (e) {
            console.log(e);
          }
        } else {
          alert("not here");
        }
      } else {
        alert("not here");
      }
    },
    async getUserData() {
      const response = await servicesModule5
        .getFundProfileReviewDetails(data)
        .then((res) => {
          this.getDataUser = res;

          console.log(this.getDataUser);
        });
    },
    handleClick(newTab) {
      this.currentTab = newTab;
    }, // end tab
    onModelUpdated(newVal, schema) {
      console.log(schema);
      if (schema == "document") {
        console.log(newVal.replace("C:\\fakepath\\", ""));
        this.model.document = newVal.replace("C:\\fakepath\\", "");
      }
    },
  },
};
</script>

<style lang="scss" scoped>
.default-tabs {
  position: relative;
  margin: 0 auto;
  &__item {
    display: inline-block;
    margin: 0 5px;
    padding: 10px;
    padding-bottom: 8px;
    font-size: 16px;
    letter-spacing: 0.8px;
    color: gray;
    text-decoration: none;
    border: none;
    background-color: transparent;
    border-bottom: 2px solid transparent;
    cursor: pointer;
    transition: all 0.25s;
    &_active {
      color: black;
    }
    &:hover {
      border-bottom: 2px solid gray;
      color: black;
    }
    &:focus {
      outline: none;
      border-bottom: 2px solid gray;
      color: black;
    }
    &:first-child {
      margin-left: 20px;
    }
    &:last-child {
      margin-right: 0;
    }
  }
  &__active-line {
    position: absolute;
    bottom: 0;
    left: 0;
    height: 2px;
    background-color: black;
    transition: transform 0.4s ease, width 0.4s ease;
  }
}
</style>
