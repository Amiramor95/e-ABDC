...
<!--                        </tr>-->
          ...
          <section slot="pdf-content">
            ...
            <div>
              ...
              <div>
                ...
                <table class="table table-striped">
                  <tbody
                    v-for="(item, index) in distributorDetail.DATAFP"
                    :key="index"
                  >
                    <div
                      v-if="
                        (distributorDetail.DIST_TYPE === 4) ||
                          (distributorDetail.DIST_TYPE === 5)
                      "
                    >
                      <tr>
                        <td style="width: 20%">Financial Institution</td>
                  ...
                  >
                    ...
                    >
                      ...
                      <tr>
                        <td>Mobile Phone Number</td>
                        <td>:</td>
                        <td class="type1" v-if="item.ADD_MOBILE_NUMBER !== null && item.ADD_MOBILE_NUMBER != '-'">
        ...
        <div>
          ...
          <div class="float-right">
            ...
            <button
              @click="submit(1, 'Are you sure to save as Draft?')"
              type="button"
            ...
            <button
              ...
              @click="submit(8, 'Are you sure to return?')"
              v-show="isShowReturn"
            >
              <i class="fa fa-reply" /> &nbsp; Return
            </button>
            <button
              @click="submit(15, 'Are you sure to approve?')"
              v-show="isShowSubmit"
              type="button"
...
export default {
  ...
  mounted() {
    var data = localStorage.getItem(
      "distributor-UpdateDetails-SubmissionDetails-secondApproval"
    );
  ...
  methods: {
    async back() {
      this.$router.push(
        "/distributor/distributor-UpdateDetails-SubmissionList-secondApproval"
      );
    },
    download() {
    ...
    async submit(status, confirm_text) {
      if(confirm(confirm_text)){
        ...
        data.append("USER_TYPE", "SECONDLEVEL");
        console.log(status);

        if(status == 1){
          this.isLoad = true;
          data.append("APPROVAL_STATUS_NEXT", 1); //Draft
          const response = await services06Module1.createDistributorUpdateReview(
            data
          );
          this.getDistributorById(this.approvalDetail.DIST_ID);
          //this.isLoad = false;
        }else if (status == 15) { // Approved by Dist Manager but Pending for Approval by RD.
          ...
          this.isLoad = true;
          data.append("APPR_INDEX_CURRENT", JSON.parse(distData).APPR_INDEX);
          data.append("APPROVAL_STATUS_NEXT", 15); //pending
          const response = await services06Module1.createDistributorUpdateReview(
            data
          );
          this.isLoad = false;
          alert("This submission has been approved, please wait for review.");
          this.$router.go(-1);
        } else if (status == 8) {
          //console.log(JSON.parse(distData).APPR_INDEX);
          this.isLoad = true;
          data.append("APPROVAL_STATUS_NEXT", 8); //return
          const response = await services06Module1.createDistributorUpdateReview(
            data
          );
    ...
    async getDistributorById(dist_id) {
      ...
      if (this.approvalDetail.TS_ID == 2 || this.approvalDetail.TS_ID == 1 ) { // 2=New Entry, 1=Draft by 2nd Level, 42= Rejected by FIMM, 7 = Returned by FIMM
    ...
    async getDistributorApprovalRemark() {
      const user = localStorage.getItem("user");
      const response = await services06Module1.getReviewRemark(this.approvalDetail.DIST_TEMP_ID, 3);
