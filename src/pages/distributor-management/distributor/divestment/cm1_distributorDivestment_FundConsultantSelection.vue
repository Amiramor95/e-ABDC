...
export default {
  ...
  methods: {
    ...
    Verify: async function () {
      ...
      if (this.model.Distributor.DISTRIBUTOR_ID == null) {
        ...
        try {
          ...
          this.$router.push("/distributor/DistributorDivestment-FundConsultant-consultant");
    ...
    getDistributorList: async function () {
      const user = localStorage.getItem("user");
      const response = await servicesModule1.getDistributor_fundConsultantDivestment(
        JSON.parse(user).user_id
      );
