...
import * as fundSelection from "./cm1_distributorDivestment_FundSelection";
...
export default {
  mounted() {
    ...
    var dive_id = localStorage.getItem("DIVE_ID");
    this.DIVESTMENTDETAILS = JSON.parse(data);
    this.DIVE_ID = JSON.parse(dive_id);
  ...
  methods: {
    ...
    getDistributorMergeDetails: async function() {
      ...
      if (JSON.stringify(this.mergerdetails.CEASE_STATUS) == 1) {
        this.visibleMergerDetails = true;
      }

      if (this.mergerdetails.SECOND_LEVEL != null) {
        this.model.SecondLevelAgreement = JSON.stringify(
          this.mergerdetails.SECOND_LEVEL
        );
      }
      if (JSON.stringify(this.mergerdetails.SECOND_LEVEL) == 1) {
        this.visibleSecondLevel = true;
      }

      if (this.mergerdetails.CESSATION_DATE != null) {
        this.model.cessationDate = moment(
          this.mergerdetails.CESSATION_DATE
        ).format("YYYY-MM-DD");
      }

      if (this.mergerdetails.LEGAL_DATE != null) {
        this.model.LegalDay = moment(this.mergerdetails.LEGAL_DATE).format(
          "YYYY-MM-DD"
        );
      }
    },
    ...
    getFundRecord: async function() {
      ...
      // const response = await servicesModule1.Submission_fund_list(JSON.parse(dive_id));
      const user = localStorage.getItem("user");
    ...
    getDocumentByIdSecondLevel: async function () {
      // alert("here")
      const dive_id = localStorage.getItem("DIVE_ID");
      const response = await servicesModule1.getDocumentByIdSecondLevel(JSON.parse(dive_id));
      // alert(JSON.stringify(response));
      response.forEach((element) => {
        console.log(element.DOC_BLOB);
        element.DOC_BLOB = this.b64toBlobPDF(
    ...
    back() {
     ...
     this.UnselectAll();
      this.$router.push("/distributor/DistributorDivestment-EditFund");
    },
    ...
    Submission: async function() {
      if (confirm("Are you sure you want to proceed this submission?")) {
        ...
        data.append(
          "NOTI_REMARK",
          "(FUND DIVESTMENT) New entry pending for Approval"
        );
        ...
        try {
          ...
          if (this.model.fileOne.length == 0) {
            ...
            if (
              this.model.SecondLevelAgreement == 1 &&
              this.model.fileSecondLevel.length == 0
            ) {
              alert("Document for Second Level Agreement");
            } else {
              if (this.RecordCount == 0 || this.RecordCount == null) {
                alert("Please Select Fund to Divest");
              } else {
                const response = await servicesModule1.fund_submission(data);
                this.$router.push("/distributor/DistributorDivestment-List");
              }
