    ...
    <va-card>
      <div class="ml-3 col-md-12">
        ...
        <div class="col-ml-2">
          <va-card>
            <va-data-table
              :fields="RecordConsultantFields"
              :data="RecordFilteredConsultantData"
              no-data-label="No data found"
...
export default {
  ...
  computed: {
    ...
    RecordFilteredConsultantData() {
      if (!this.term || this.term.length < 1) {
        return this.RecordConList;
      }
      return this.RecordConList.filter((item) => {
        return this.term
          .toLowerCase()
          .split(" ")
          .every(
   ...
   getFundRecord: async function () {
    ...
    returnSubmission: async function () {
      if (confirm("Are you sure you want to return this submission?")) {
        if(this.model.comment==null){
          alert('Comment field is required for return')
        }
        else {
          const data = new FormData();
          data.append("DIVE_DIST_APPR_ID", this.DIVESTMENTDETAILS.DIVE_DIST_APPR_ID);
          ...
          data.append("NOTI_REMARK", "(FUND & CONSULTANT DIVESTMENT) This Entry has been returned by Manager");
          ...
          try {
            ...
            this.$router.push("/fimm/suspension-hodApproval-list");
          } catch (error) {
            console.log(error);
          }
        }
    ...
    reviewSubmission: async function () {
      if (confirm("Are you sure you want to proceed this submission?")) {
        ...
        data.append("NOTI_REMARK", "(FUND & CONSULTANT DIVESTMENT) New entry pending for Recommendation");
    ...
    returnSubmission: async function () {
      if (confirm("Are you sure you want to retun this submission?")) {
        const data = new FormData();
        data.append("DIVE_DIST_APPR_ID",this.DIVESTMENTDETAILS.DIVE_DIST_APPR_ID);
        ...
        data.append("NOTI_REMARK", "(FUND & CONSULTANT DIVESTMENT) Returned Submission");
    ...
    rejectSubmission: async function () {
      if (confirm("Are you sure you want to reject this submission?")) {
        ...
        data.append("NOTI_LOCATION", "DistributorDivestment-List");
        data.append("NOTI_REMARK", "(FUND & CONSULTANT DIVESTMENT) Rejected Submission");
