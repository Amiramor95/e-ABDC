<template>
  <va-card>
    <br />
    <h4 slot="header" class="card-title">Fund and Consultant Divestment Submission</h4>
    ...
    <va-card>
      <div class="ml-3 col-md-12">
          ...
          <div class="ml-4">
            <table class="table table-striped">
              <tbody>
                ...
                <tr v-if="DIVESTMENTDETAILS.TS_ID == 15 || DIVESTMENTDETAILS.TS_ID == 7 || DIVESTMENTDETAILS.TS_ID == 5 ">
                  <td style="width: 35%">
                     Cease Operation
                  </td>
                ...
                <!-- <tr v-if="DIVESTMENTDETAILS.TS_ID == 15 && DIVESTMENTDETAILS.CEASE_STATUS ==1">
                  <td style="width: 35%">
                     Cessation Date
                  </td>
                  <td>: {{ DIVESTMENTDETAILS.CESS_DATE_DISPLAY }}</td>
                </tr>
                <tr v-if="DIVESTMENTDETAILS.TS_ID == 15 && DIVESTMENTDETAILS.CEASE_STATUS ==1">
                  <td style="width: 35%">
                   Legal Day (1)
                  </td>
                  <td>: {{ DIVESTMENTDETAILS.LEG_DATE_DISPLAY }}</td>
                </tr> -->
              </tbody>
            </table>
            <br />
            <vue-form-generator v-if="DIVESTMENTDETAILS.TS_ID == 1"
              :model="model"
        ...
        <div>
          ...
          <!-- view second Level -->
          <vue-form-generator  v-if="DIVESTMENTDETAILS.TS_ID == 15 || DIVESTMENTDETAILS.TS_ID == 5"
            :model="model"
    ...
    <div class="float-right" v-if="DIVESTMENTDETAILS.TS_ID == 1">
      <button
      @click="SaveAsDraft"
...
export default {
  mounted() {
    ...
    this.model.cessationDate = moment(this.DIVESTMENTDETAILS.CESSATION_DATE).format(
        "YYYY-MM-DD"
      );
  ...
  methods: {
    ...
    getFundRecord: async function () {
      ...
      const response = await servicesModule1.Submission_fund_list(
    ...
    getAllRecordConsultant: async function () {
      const user = localStorage.getItem("user");
      const dive_id = localStorage.getItem("test");
      const response = await servicesModule1.Submission_cons_list(
        JSON.parse(dive_id).DIVE_ID
      );
    ...
    Submission: async function () {
      ...
      //(processflowID,approval_level index)
        ...
        data.append("NOTI_REMARK", "(FUND & CONSULTANT DIVESTMENT) New entry pending for Approval");
