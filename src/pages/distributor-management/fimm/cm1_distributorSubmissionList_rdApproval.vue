      ...
      >
        ...
        <template slot="actions" slot-scope="props">
          <h4 class="mt-2">
            <span
              v-if="
                props.rowData.TS_PARAM_R === 'NEW ENTRY' ||
                  props.rowData.TS_PARAM_R === 'DRAFT' ||
                  props.rowData.TS_PARAM_R === 'ACCEPTED' ||
                  props.rowData.TS_PARAM_R === 'PENDING' ||
                  props.rowData.TS_PARAM_R === 'RETURNED'
              "
              v-b-tooltip.hover
              title="detail"
  ...
  methods: {
    ...
    details (data) {
      ...
      localStorage.setItem(
        'distributor-SubmissionList-rdApproval',
        JSON.stringify(data),
      )
      this.$router.push({
        ...
        return this.term
          ...
          .every(
            v =>
              ...
              (item.PAYMENT_STATUS ? item.PAYMENT_STATUS.toLowerCase().includes(v) : false) ||
              (item.TS_PARAM_R ? item.TS_PARAM_R.toLowerCase().includes(v) : false),
          )
      })
    },
  },
