...
export default {
  mounted() {
    var data = localStorage.getItem("view-revocationSubmission-hod");
  ...
  methods: {
    ...
    saveAsDraft: async function () {
      if (confirm("Are you sure you want to save this as draft?")) {
        ...
        try {
          ...
          this.$router.push("/fimm/revocation-hodApproval-list");
        } catch (error) {
          console.log(error);
        }
      }
    ...
    reviewSubmission: async function () {
      if (confirm("Are you sure you want to proceed this submission?")) {
        ...
        data.append("NOTI_LOCATION", "revocation-gmApproval-list");
        var NotificationNote =
          "(REVOCATION) " +
          this.DISTDATA.DIST_NAME +
