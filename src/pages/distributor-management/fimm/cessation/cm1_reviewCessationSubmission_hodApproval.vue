...
export default {
  mounted() {
    var data = localStorage.getItem("cessation-submission-hodApproval");
    ...
    this.getAuditLog();
    this.getDocumentByFimmApprover();
  ...
  methods: {
    ...
    getDocumentByFimmApprover: async function () {
      const user = localStorage.getItem("user");
      const response = await servicesModule1.getDocumentByFimmApprover(
        this.DISTDATA.CESSATION_ID,
    ...
    saveAsDraft: async function () {
      if (confirm("Are you sure you want to save this as draft?")) {
        ...
        data.append(
          "CESSATION_FIMM_APPROVAL_ID",
          this.DISTDATA.CESSATION_FIMM_APPROVAL_ID
        );

        data.append("APPR_REMARK", this.model.comment);
        data.append("APPR_PUBLISH_STATUS", 0);
        data.append("FIMM_TS_ID", 1);
        const user = localStorage.getItem("user");
        ...
        try {
          const response = await servicesModule1.fimmUpdateApproval(data);
          const response1 = await servicesModule1.uploadFimmCessationDocument(
            data
          );
          this.$router.push("/fimm/cessation-submission-hod-list");
        } catch (error) {
          console.log(error);
        }
      }
    ...
    returnSubmission: async function () {
      if (confirm("Are you sure you want to return this submission?")) {
        ...
        //admin rd dashboard notification
        ...
        var NotificationNote =
          "(CESSATION) " +
          this.DISTDATA.DISTRIBUTOR_NAME + ".This entry has been returned by Hod";
    ...
    reviewSubmission: async function () {
      ...
      if (confirm("Are you sure you want to approve this submission?")) {
        ...
        //check cease date
        if (this.DISTDATA.CESSATION_DATE != null) {
          //same or before
          if (moment(this.DISTDATA.CESSATION_DATE).isSameOrBefore(moment())) {
            data.append("CEASE_ACTION", "TRUE");
            data.append(
              "NOTI_EMAIL",
              "Your cessation submission has been Approved by FIMM. All access has been revoked :"
            );
          } else if (moment(this.DISTDATA.CESSATION_DATE).isAfter(moment())) {
            //date after approval date
            data.append("CEASE_ACTION", "FALSE");
            data.append(
              "CEASE_NOTIFICATION",
              moment(this.DISTDATA.CESSATION_DATE)
                .add(1, "days")
                .format("YYYY-MM-DD")
            );
            data.append(
              "NOTI_EMAIL",
              "Your cessation submission has been Approved by FIMM. All access will be revoked start from your cessation date:"
            );
          }
        }
        ...
        //1.email notification to distributor admin

        data.append("DISTRIBUTOR_NAME", this.DISTDATA.DISTRIBUTOR_NAME);
        ...
        data.append("CESSATION_NAME", this.DISTDATA.CESSATION_NAME);
        data.append("CESSATION_DATE", this.DISTDATA.CESSATION_DATE);

        //2. send notification to staff RD
        const approvalLevel = await services06Module0.getApprovalLevel(3, 1);
        data.append("APPR_LIST", JSON.stringify(approvalLevel));
        data.append("NOTI_LOCATION", "cessation-submission-overview-list");

        var NotificationNote =
          "(CESSATION) " +
          this.DISTDATA.DISTRIBUTOR_NAME +
          ".This submission has been approved by HOD";
        data.append("NOTI_REMARK", NotificationNote);

        //3. send notification to finance - prepayment topup
        ...
        var NotificationNote2 =
          "(REFUND) " +
          this.DISTDATA.DISTRIBUTOR_NAME +
          ".This distributor has been terminated";
        data.append("NOTI_REMARK2", NotificationNote2);

        //4. send notification to ID to close fund
        ...
        try {
          const response = await servicesModule1.fimmUpdateHodApproval(data);

          if (moment(this.DISTDATA.CESSATION_DATE).isSameOrBefore(moment())) {
            const response1 = await servicesModule1.actionCease(data);
          }

          const response2 = await servicesModule1.uploadFimmCessationDocument(
            data
          );
          this.$router.push("/fimm/cessation-submission-hod-list");
        } catch (error) {
            ...
            fields: [
              {
                ...
                text: "Choose a File",
                model: "file",
                type: "vfg-custom-multiple-file-edit-fimmcease",
                styleClasses: "col-md-12",
