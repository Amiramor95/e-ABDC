<template>
  <va-card>
    <br />
    <span v-if="DIVESTMENTDETAILS.DIVE_TYPE == '35' "><h4 slot="header" class="card-title">Fund Divestment Submission</h4></span>
    <span v-if="DIVESTMENTDETAILS.DIVE_TYPE == '36' "><h4 slot="header" class="card-title">Consultant Divestment Submission</h4></span>
    <span v-if="DIVESTMENTDETAILS.DIVE_TYPE == '37' "><h4 slot="header" class="card-title">Fund and Consultant Divestment Submission</h4></span>
...
export default {
  ...
  methods: {
    ...
    getDistributorMergeDetails: async function () {
      ...
      const response = await servicesModule1.distributorMergeDetails(JSON.parse(divestmentdetails).DIVE_ID);
   ...
   getFundRecord: async function () {
    ...
    // back button
    back() {
      this.$router.push("/fimm/distributor-DivestmentList-rdApproval");
    },
    ...
    SaveAsDraft: async function () {
      ...
      if (this.model.FileSecondReview.length != 0) {
        ...
        try {
          const response = await servicesModule1.update_Approvalrd(data);
          this.$router.push("/fimm/distributor-DivestmentList-rdApproval");
        } catch (error) {
          console.log(error);
        }
    ...
    reviewSubmission: async function () {
      if (confirm("Are you sure you want to proceed this submission?")) {
        ...
        const approvalLevel = await services06Module0.getApprovalLevel(5, 2);
        data.append("APPR_LIST", JSON.stringify(approvalLevel));

        data.append("NOTI_LOCATION", "distributor-DivestmentList-Hod-rdApproval");
        if (this.DIVESTMENTDETAILS.DIVE_TYPE == '35'){
          data.append("NOTI_REMARK","(FUND DIVESTMENT) Pending for Approval");
        } else if (this.DIVESTMENTDETAILS.DIVE_TYPE == '36'){
          data.append("NOTI_REMARK","(CONSULTANT DIVESTMENT) Pending for Approval");
        }else {
          data.append("NOTI_REMARK","(FUND & CONSULTANT DIVESTMENT) Pending for Approval");
        }
    ...
    returnSubmission: async function () {
      if(this.model.comment.length>0) {
        ...
        if (confirm("Are you sure you want to return this submission?")) {
          ...
          data.append("APPR_REMARK", this.model.comment);
          data.append("APPR_PUBLISH_STATUS", "1");
          data.append("TS_ID", "28"); // return submission by RD

          const user = localStorage.getItem("user");
          data.append("CREATE_BY", JSON.parse(user).user_id);

          //Send to 2nd approval
          ...
          if (this.DIVESTMENTDETAILS.DIVE_TYPE == '35') {
            data.append("NOTI_REMARK", "(FUND DIVESTMENT) Returned Submission By RD");
          } else if (this.DIVESTMENTDETAILS.DIVE_TYPE == '36') {
            data.append("NOTI_REMARK", "(CONSULTANT DIVESTMENT) Returned Submission By RD");
          } else {
            data.append("NOTI_REMARK", "(FUND & CONSULTANT DIVESTMENT) Returned Submission By RD");
          }
    ...
    rejectSubmission: async function () {
      if(this.model.comment.length>0) {
        ...
        if (confirm("Are you sure you want to reject this submission?")) {
          ...
          data.append("NOTI_LOCATION", "DistributorDivestment-ApprovalList");
          if (this.DIVESTMENTDETAILS.DIVE_TYPE == '35') {
            data.append("NOTI_REMARK", "(FUND DIVESTMENT) Rejected Submission By RD");
          } else if (this.DIVESTMENTDETAILS.DIVE_TYPE == '36') {
            data.append("NOTI_REMARK", "(CONSULTANT DIVESTMENT) Rejected Submission By RD");
          } else {
            data.append("NOTI_REMARK", "(FUND & CONSULTANT DIVESTMENT) Rejected Submission By RD");
          }
