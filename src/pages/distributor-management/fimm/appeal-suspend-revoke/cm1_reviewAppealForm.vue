  ...
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">
        Suspension/Revocation : Review Submission Form
      </h4>
    </div>
    <br />
    <br />
...
export default {
  mounted() {
    var data = localStorage.getItem("review-appeal-submission");
  ...
  methods: {
    ...
    saveAsDraft: async function () {
      if (confirm("Are you sure you want to save this as draft?")) {
        ...
        try {
          const response = await servicesModule1.updateAppealReview(data);
          const response1 = await servicesModule1.createDocumentAppealApproval(
            data
          );
          this.$router.push("/fimm/appeal-suspension-revocation-list");
        } catch (error) {
          console.log(error);
        }
      }
    ...
    returnSubmission: async function () {
      if (confirm("Are you sure you want to return this submission?")) {
        ...
        data.append(
          "DIST_NOTI_REMARK",
          "Your Appeal Submission has been returned by FIMM"
        );
    ...
    reject: async function () {
      if (confirm("Are you sure you want to reject this submission?")) {
        ...
        data.append(
          "NOTI_EMAIL",
          "Your appeal submission has been Rejected by FIMM. All access has been revoked :"
        );
        ...
        data.append("EFFECTIVE_DATE", this.DISTDATA.EFFECTIVEDATE);
        ...
        try {
          if (this.DISTDATA.SUBMISSION_TYPE == "SUSPENSION") {
            const response = await servicesModule1.updateAppealReview(data);
            const response1 =
              await servicesModule1.createDocumentAppealApproval(data);
          }
          if (this.DISTDATA.SUBMISSION_TYPE == "REVOCATION") {
            const response = await servicesModule1.updateAppealRejectRevoke(
              data
            );
    ...
    reviewSubmission: async function () {
      ...
      if (confirm("Are you sure you want to accept this submission?")) {
        ...
        if (this.DISTDATA.SUBMISSION_TYPE == "SUSPENSION") {
          data.append("NOTI_LOCATION", "appealSuspension-hodApproval-list");

          var NotificationNote =
            "(APPEAL) " +
            this.DISTDATA.DIST_NAME +
            ".New entry pending for Recommendation";
          data.append("NOTI_REMARK", NotificationNote);
        }
        if (this.DISTDATA.SUBMISSION_TYPE == "REVOCATION") {
          data.append("NOTI_LOCATION", "appealRevocation-hodApproval-list");
          var NotificationNote =
            "(APPEAL) " +
            this.DISTDATA.DIST_NAME +
            ".New entry pending for Recommendation";
