...
export default {
  mounted() {
    var data = localStorage.getItem("view-appealRevocation-hod");
    this.DISTDATA = JSON.parse(data);
    //alert(this.DISTDATA.EFFECTIVE_DATE );
  ...
  methods: {
    ...
    saveAsDraft: async function () {
      if (confirm("Are you sure you want to save this as draft?")) {
        ...
        try {
          ...
          this.$router.push("/fimm/appealRevocation-hodApproval-list");
        } catch (error) {
          console.log(error);
        }
      }
    ...
    returnSubmission: async function () {
      if (confirm("Are you sure you want to return this submission?")) {
        ...
        data.append("NOTI_LOCATION", "appeal-suspension-revocation-list");
         var NotificationNote ="(APPEAL) " +this.DISTDATA.DIST_NAME +".This Entry has been returned by HOD";
     ...
     reject: async function () {
      if (confirm("Are you sure you want to reject this submission?")) {
        ...
        var NotificationNote ="(APPEAL) " +this.DISTDATA.DIST_NAME +".This Entry has been rejected by HOD";
        data.append("NOTI_REMARK", NotificationNote);
    ...
    reviewSubmission: async function () {
      ...
      if (confirm("Are you sure you want to recommend for approval this submission?")) {
        ...
        const approvalLevel = await services06Module0.getApprovalLevel(4, 3);
        data.append("APPR_LIST", JSON.stringify(approvalLevel));


        if(this.DISTDATA.SUBMISSION_TYPE == "SUSPENSION"){
            data.append("NOTI_LOCATION", "appealRevocation-gmApproval-list");
            
        var NotificationNote ="(APPEAL) " +this.DISTDATA.DIST_NAME +".New entry pending for Recommendation";
        data.append("NOTI_REMARK", NotificationNote);
        };
