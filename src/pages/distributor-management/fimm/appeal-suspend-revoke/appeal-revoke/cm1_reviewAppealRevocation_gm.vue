  ...
  <va-card>
    ...
    <div class="float-right">
      ...
      <button
        ...
        class="ml-1 btn btn-fill btn-md"
      >
        <i class="fa fa-paper-plane" /> &nbsp; Recommend for approval
      </button>
    </div>
  </va-card>
</template>
...
export default {
  mounted() {
    var data = localStorage.getItem("view-appealRevocation-gm");
  ...
  methods: {
    ...
    saveAsDraft: async function () {
      if (confirm("Are you sure you want to save this as draft?")) {
        ...
        try {
          ...
          const response1 = await servicesModule1.createDocumentAppealApproval(
            data
          );
          this.$router.push("/fimm/appealRevocation-gmApproval-list");
        } catch (error) {
          console.log(error);
        }
      }
    ...
    returnSubmission: async function () {
      if (confirm("Are you sure you want to return this submission?")) {
        ...
        var NotificationNote ="(APPEAL) " +this.DISTDATA.DIST_NAME +".This Entry has been returned by GM";
        ...
        var NotificationNote2 ="(APPEAL) " +this.DISTDATA.DIST_NAME +".This Entry has been returned to Reviewer by GM";
    ...
    reject: async function () {
      if (confirm("Are you sure you want to reject this submission?")) {
        ...
        var NotificationNote ="(APPEAL) " +this.DISTDATA.DIST_NAME +".This Entry has been rejected by GM";
        data.append("NOTI_REMARK", NotificationNote);
    ...
    reviewSubmission: async function () {
      ...
      if (
        confirm(
          "Are you sure you want to recommend for approval this submission?"
        )
      ) {
        const data = new FormData();
        ...
        const approvalLevel = await services06Module0.getApprovalLevel(4, 4);
        data.append("APPR_LIST", JSON.stringify(approvalLevel));

        if (this.DISTDATA.SUBMISSION_TYPE == "SUSPENSION") {
          data.append("NOTI_LOCATION", "appealSuspension-ceoApproval-list");
          var NotificationNote ="(APPEAL) " +this.DISTDATA.DIST_NAME +".New entry pending for Approval";
          data.append("NOTI_REMARK", NotificationNote);
        }
        if (this.DISTDATA.SUBMISSION_TYPE == "REVOCATION") {
          data.append("NOTI_LOCATION", "appealRevocation-ceoApproval-list");
          var NotificationNote ="(APPEAL) " +this.DISTDATA.DIST_NAME +".New entry pending for Approval";
          data.append("NOTI_REMARK", NotificationNote);
        }
        ...
        data.append(
          "NOTI_EMAIL",
          "This Appeal Submission pending for your approval :"
        );
