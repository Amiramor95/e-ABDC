  ...
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">
        Revocation : Review Appeal Submission Form
      </h4>
    </div>
    <br />
    <br />
...
export default {
  mounted() {
    var data = localStorage.getItem("view-appealRevocation-ceo");
    ...
    this.getAppealComment();
    this.getDocumentByAppealApprover();
  ...
  methods: {
    ...
    getAppealComment: async function () {
      const user = localStorage.getItem("user");
      const response = await servicesModule1.getAppealComment(
      this.DISTDATA.SUSPEND_REVOKE_APPEAL_ID,JSON.parse(user).USER_GROUP_ID
      );
      if(response.APPR_REMARK != null || response.APPR_REMARK != ""){
    ...
    getDocumentByAppealApprover: async function () {
      const user = localStorage.getItem("user");
      const response = await servicesModule1.getDocumentByAppealApprover(
        this.DISTDATA.SUSPEND_REVOKE_APPEAL_ID,
    ...
    saveAsDraft: async function () {
      if (confirm("Are you sure you want to save this as draft?")) {
        ...
        data.append("SUSPEND_REVOKE_APPEAL_ID", this.DISTDATA.SUSPEND_REVOKE_APPEAL_ID);
        data.append("SUSPEND_REVOKE_APPEAL_APPR_ID", this.DISTDATA.SUSPEND_REVOKE_APPEAL_APPR_ID);

        data.append("APPR_REMARK", this.model.comment);
        data.append("APPR_PUBLISH_STATUS", "0");
        data.append("TS_ID", "1");
         data.append("FIMM_ISSUBMIT", "0");
        const user = localStorage.getItem("user");
        ...
        try {
          const response = await servicesModule1.updateAppealApproval(data);
          const response1 = await servicesModule1.createDocumentAppealApproval(data);
          this.$router.push("/fimm/appealRevocation-ceoApproval-list");
        } catch (error) {
          console.log(error);
        }
      }
    ...
    returnSubmission: async function () {
      if (confirm("Are you sure you want to return this submission?")) {
        ...
        data.append("TS_ID", "9");
        data.append("FIMM_ISSUBMIT", "1");
        const user = localStorage.getItem("user");
        data.append("CREATE_BY", JSON.parse(user).user_id);
        data.append("APPR_GROUP_ID",JSON.parse(user).USER_GROUP_ID);


         //rd dashboard notification
        ...
        var NotificationNote ="(APPEAL) " +this.DISTDATA.DIST_NAME +".This Entry has been returned by CEO";
       ...
       data.append("NOTIDASH2", 1);
        ...
        data.append("NOTI_LOCATION2", "overview-appeal-suspension-revocation-list");
         var NotificationNote2 ="(APPEAL) " +this.DISTDATA.DIST_NAME +".This Entry has been returned to Reviewer by CEO";
      ...
      if (confirm("Are you sure you want to reject this submission?")) {
        ...
        data.append(
          "DIST_NOTI_REMARK",
          "Your Appeal Submission has been rejected by FIMM"
        );


        //email notification
         data.append("NOTI_EMAIL","Your appeal submission has been Rejected by FIMM. All access has been revoked :");
        ...
        data.append("SUBMISSION_TYPE", "APPEAL");
        data.append("DATE_START", "");
        data.append("DATE_END", "N/A");
        data.append("EFFECTIVE_DATE", this.DISTDATA.EFFECTIVE_DATE );
        data.append("REASON", "N/A");

        data.append("RD_NOTI",1);

         const approvalLevel = await services06Module0.getApprovalLevel(4, 1);
        data.append("APPR_LIST", JSON.stringify(approvalLevel));
        data.append("NOTI_LOCATION", "suspension-revocation-list");

        var NotificationNote ="(APPEAL) " +this.DISTDATA.DIST_NAME +".This Entry has been rejected by CEO";
        data.append("NOTI_REMARK", NotificationNote);
        ...
        try {
          const response = await servicesModule1.updateAppealRejectRevoke(data);
          const response1 = await servicesModule1.actionRevoke(data);
          const response2 = await servicesModule1.createDocumentAppealApproval(data);
          this.$router.push("/fimm/appealRevocation-ceoApproval-list");
        } catch (error) {
          console.log(error);
        }
    ...
    reviewSubmission: async function () {
      //ts_id = 3 (approved)
      if (confirm("Are you sure you want to approved this submission?")) {
        const data = new FormData();
        ...
        data.append(
          "SUSPEND_REVOKE_APPEAL_APPR_ID",
          this.DISTDATA.SUSPEND_REVOKE_APPEAL_APPR_ID
        );
        ...
        data.append("APPR_PUBLISH_STATUS", "1");
        data.append("TS_ID", "3");
        data.append("FIMM_TS_ID", "3");
        data.append("FIMM_ISSUBMIT", "1");
        const user = localStorage.getItem("user");
        data.append("CREATE_BY", JSON.parse(user).user_id);
        data.append("APPR_GROUP_ID", JSON.parse(user).USER_GROUP_ID);

        //dashboard notification to rd
        const approvalLevel = await services06Module0.getApprovalLevel(4, 1);
        data.append("APPR_LIST", JSON.stringify(approvalLevel));
        ...
        if (this.DISTDATA.SUBMISSION_TYPE == "SUSPENSION") {
          ...
          var NotificationNote ="(APPEAL)  " +this.DISTDATA.DIST_NAME +".This Entry has been approved by CEO";
        data.append("NOTI_REMARK", NotificationNote);
        }
        if (this.DISTDATA.SUBMISSION_TYPE == "REVOCATION") {
          data.append("NOTI_LOCATION", "appeal-suspension-revocation-list");
          var NotificationNote ="(APPEAL)  " +this.DISTDATA.DIST_NAME +".This Entry has been approved by CEO";
        data.append("NOTI_REMARK", NotificationNote);
        }
        ...
        data.append(
          "DIST_NOTI_REMARK",
          "Your Appeal Submission has been approved by FIMM"
        );
        ...
        data.append("NEW_REGISTRATION_NUMBER", this.DISTDATA.DIST_REGI_NUM2);
        data.append("SUBMISSION_TYPE", "APPEAL");
        data.append("DATE_START", "N/A");
        data.append("DATE_END", "N/A");
        data.append("EFFECTIVE_DATE", "N/A");
        data.append("REASON", "N/A");
        ...
        try {
          const response = await servicesModule1.updateAppealApprovalCeo(data);
          const response1 = await servicesModule1.unSuspendDistributorConsultant(data);
          const response2 = await servicesModule1.createDocumentAppealApproval(data);
          this.$router.push("/fimm/appealSuspension-ceoApproval-list");
        } catch (error) {
          console.log(error);
        }
      }
            ...
            fields: [
              {
                ...
                text: "Choose a File",
                model: "fileNew",
                type: "vfg-custom-multiple-file-edit-appeal-approval",
                styleClasses: "col-md-12",
