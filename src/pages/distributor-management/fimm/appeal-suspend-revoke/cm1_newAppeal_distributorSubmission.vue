  ...
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">
        Suspension/Revocation : List of Appeal Submissions
      </h4>
      <br />
      ...
      >
        ...
        <template slot="actions" slot-scope="props">
          <h5 class="mt-1">
            <span
              v-if="
                props.rowData.FIMM_TS_ID == 1 ||
                  props.rowData.FIMM_TS_ID == 9 ||
                  props.rowData.FIMM_TS_ID == 2
              "
              v-b-tooltip.hover
              title="review Appeal Submission Form"
            ...
            <span
              v-b-tooltip.hover
              title="View Appeal Submission Form"
  ...
  methods: {
    ...
    async filterRecord() {
      try {
        ...
        const response = await servicesModule1.filterSuspenRevokeRecordAppealList(
          this.model
        );
        if (response != "error") {
    ...
    getAllList: async function() {
      const response = await servicesModule1.getAllListAppeal();
      this.RecordList = response;
      this.RecordCount = this.RecordList.length;
    },

    async viewRecord(data) {
      localStorage.setItem("view-appeal-submission", JSON.stringify(data));
      this.$router.push({
    ...
    async editRecord(data) {
      localStorage.setItem("review-appeal-submission", JSON.stringify(data));
      this.$router.push({
        ...
        return this.term
          ...
          .every(
            v =>
              ...
              item.APPROVAL_STATUS.toLowerCase().includes(v) ||
