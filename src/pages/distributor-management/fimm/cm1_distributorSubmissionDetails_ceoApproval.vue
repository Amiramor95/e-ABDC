              ...
              <table class="table table-striped">
                <tbody>
                  ...
                  <tr>
                    ...
                    <td v-if="this.distributorDetail.DIST_DATE_INCORP == null">
                      -
                    </td>
                    <td v-if="this.distributorDetail.DIST_DATE_INCORP != null">
                ...
                <table class="table">
                  <tbody>
                    ...
                    >
                      ...
                      <td style="width: 20%">{{ item.DIR_PASS_NUM }}</td>
      ...
      <div>
        ...
        <div
          ...
          v-if="
            ...
            this.approvalDetail.TS_PARAM === 'NEW ENTRY' ||
...
export default {
  ...
  mounted() {
    var data = localStorage.getItem("distributor-SubmissionList-ceoApproval");
  ...
  methods: {
    async back() {
      this.$router.push("/fimm/distributor-SubmissionList-ceoApproval");
    },
    ...
    async submit(status) {
      ...
      if (status == 3) {
        // alert(this.approvalDetail.APPR_INDEX + 1);
        if( confirm("Approve this application?")){
        if (this.distributorDetail.DIST_TYPE_NAME != "UTMC") {
          data.append("NOTI_LOCATION_DIST", "distributor-profile");
          data.append(
            "NOTI_MESSAGE",
            "A DISTRIBUTOR HAS BEEN APPROVED."
          );
          data.append("NOTI_TYPE","DISTRIBUTOR");

          const approvalLevel = await services06Module0.getApprovalLevel(
            1,
            this.approvalDetail.APPR_INDEX
          );
          ...
          data.append("APPROVAL_STATUS_NEXT", 3);
          dataEmail.append("EMAIL", this.distributorDetail.DIST_EMAIL);
          dataEmail.append("DIST_NAME", this.distributorDetail.DIST_NAME);
          dataEmail.append("DIST_REMARK", "has successfully been approved.");
          const sendEmailNotification = await services06Module1.sendEmailNotification(
            dataEmail
          );
          dataUpdate.append("DIST_TYPE", this.distributorDetail.DIST_TYPE_NAME);
          ...
          data.append(
            "NOTI_MESSAGE",
            "(DIST) New registration for BOD approval."
          );

          data.append("NOTI_TYPE","FIMM");
          ...
          if (this.distributorDetail.DIST_TYPE_NAME == "UTMC") {
            dataUpdate.append("DIST_ID", this.approvalDetail.DISTRIBUTOR_ID);
            dataUpdate.append("DIST_TYPE", this.distributorDetail.DIST_TYPE_NAME);
            const responseUpdate = await services06Module1.updateDistStatus(
              dataUpdate
            );
          };
        ...
        const approvalLevel = await services06Module0.getApprovalLevel(1, 4);
        data.append("APPR_LIST", JSON.stringify(approvalLevel));
        data.append("APPR_INDEX_CURRENT", this.approvalDetail.APPR_INDEX);
