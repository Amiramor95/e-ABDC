              ...
              <b-collapse id="collapse-financial" class="mt-2">
                ...
                <table class="table table-striped">
                  <tbody
                    ...
                    :key="index"
                  >
                    <div v-if="this.distributorDetail.DIST_TYPE_NAME == '4' || this.distributorDetail.DIST_TYPE_NAME == '5'">
                      <tr>
...
export default {
  ...
  mounted() {
    var data = localStorage.getItem("distributor-RegisterType-SubmissionDetails-rdApproval");
    this.approvalDetail = JSON.parse(data);
    this.getDistributorById(this.approvalDetail.DISTRIBUTOR_ID);
  },
  ...
  methods: {
    async back() {
          this.$router.push('/fimm/distributor-RegisterType-SubmissionList-rdApproval');
      },
    async details(data) {
    ...
    async submit(status) {
      ...
      const disttypeid = localStorage.getItem("distributor-RegisterType-SubmissionDetails-rdApproval");
      ...
      if (status == 14) {
        ...
        data.append(
          "NOTI_LOCATION",
          "distributor-RegisterType-SubmissionList-HODrdApproval"
        );

        data.append(
          "NOTI_MESSAGE",
          "(DIST) New Type Registration for HOD RD approval."
        );
        ...
        data.append(
          "NOTI_MESSAGE",
          "(DISTRIBUTOR) RECOMMENDED DISTRIBUTOR FOR APPROVAL HOD RD."
        );
        const approvalLevel = await services06Module0.getApprovalLevel(
          1,
          this.approvalDetail.APPR_INDEX + 1
        ...
        if (confirm("Return this application? ")) {
          data.append("APPROVAL_STATUS", 7); //return
          const response = await services06Module1.createDistributorRegisterTypeReview(data);
        } else {
          console.log("");
        }
    ...
    async getDistributorById(dist_id) {
      const response = await services06Module1.getTypeRegByDistId(dist_id);
