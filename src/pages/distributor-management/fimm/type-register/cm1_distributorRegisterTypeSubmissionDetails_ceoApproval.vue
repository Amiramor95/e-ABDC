...
export default {
  ...
  mounted() {
    // this.$modals.showInstructionModal.$show();
    var data = localStorage.getItem("distributor-RegisterType-SubmissionDetails-ceoApproval");
  ...
  methods: {
    ...
    async submit(status) {
      ...
      const disttypeid = localStorage.getItem("distributor-RegisterType-SubmissionDetails-ceoApproval");
      ...
      if (status == 3) {
        ...
        if( confirm("Approve this application?")) {
          ...
          if (this.approvalDetail.DIST_TYPE_NAME != "UTMC") {

            data.append("NOTI_LOCATION_DIST", "distributor-profile");
            data.append(
              "NOTI_MESSAGE",
              "A DISTRIBUTOR TYPE HAS BEEN APPROVED."
            );
            ...
            const sendEmailNotification = await services06Module1.sendEmailNotification(dataEmail);
          } else {
            const approvalLevel = await services06Module0.getApprovalLevel(
              1,
            ...
            data.append(
              "NOTI_LOCATION",
              "distributor-RegisterType-SubmissionList-boardApproval"
            );

            data.append(
              "NOTI_MESSAGE",
              "(DIST) New Type Registration for BOD approval."
            );
          ...
          if (this.distributorDetail.DIST_TYPE_NAME != "UTMC") {
            ...
            const responseUpdate = await services06Module1.updateDistStatus(dataUpdate);
          }
      ...
      this.$router.push('/fimm/distributor-RegisterType-SubmissionList-ceoApproval');
