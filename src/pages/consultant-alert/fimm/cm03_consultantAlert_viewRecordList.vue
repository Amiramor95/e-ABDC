<template>
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">List of Consultant Alert Records</h4>
      <br />
...
export default {
  ...
  methods: {
    ...
    back () {
      ...
      if (JSON.parse(user).USER_GROUP_ID == 12){
      this.$router.push({ name: "consultantAlert-hodLraRecordList",});
      }else if (JSON.parse(user).USER_GROUP_ID == 13){
      this.$router.push({ name: "consultantAlert-gmLraRecordList",});
      }
    }, // back button
    ...
    async viewRecord(data) {
       localStorage.setItem("consultantAlert-viewConsultantCaRecord",JSON.stringify(data));
       this.$router.push({
