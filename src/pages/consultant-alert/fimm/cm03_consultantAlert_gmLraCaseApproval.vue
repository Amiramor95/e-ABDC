<template>
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">Consultant Alert : Case for Approval</h4>
      <br />
      <br />
    </div>
    <va-card>
...
export default {
  mounted() {
    var data = localStorage.getItem("consultantAlert-gmLraCaseApproval");
  ...
  methods: {
    ...
    saveAsDraft: async function () {
      ...
      try {
        ...
        this.$router.push("/fimm/consultantAlert-gmLraRecordList");
      } catch (error) {
        console.log(error);
      }
      }
    ...
    verifyCase: async function () {
      ...
      if (this.model.endDate != null) {
        ...
        if (moment(this.model.endDate).subtract(21, "days").isAfter(this.model.startDate))
        {
         // alert( moment(this.model.endDate).subtract(21, "days").format("YYYY-MM-DD"));
          data.append( "THREE_WEEKS_NOTIFICATION",moment(this.model.endDate).subtract(21, "days").format("YYYY-MM-DD"));
          data.append( "THREE_DAYS_NOTIFICATION",moment(this.model.endDate).subtract(3, "days").format("YYYY-MM-DD"));

        }else if (moment(this.model.endDate).subtract(21, "days").isSameOrBefore(this.model.startDate))
        {
           data.append( "THREE_DAYS_NOTIFICATION",moment(this.model.endDate).subtract(3, "days").format("YYYY-MM-DD"));
        }
      }
      ...
      var remarkEntry = "(CA)This entry has been approved by GM for " + code;
      ...
      var notiTitle = "This entry has been approved by General Manager";
    ...
    returnCase: async function () {
      if (confirm("Are you sure you want to return this entry?")) {
        ...
        //return direct to 1st level and email to both 1st & 2nd level
        const approvalLevel = await services06Module0.getApprovalLevel(6, 1);
        data.append("APPR_LIST", JSON.stringify(approvalLevel));

        const approvalLevel2 = await services06Module0.getApprovalLevel(6, 2);
        data.append("APPR_LIST2", JSON.stringify(approvalLevel2));

        //notification
        data.append("NOTI_LOCATION", "consultantAlert-overviewList");
        data.append("NOTI_LOCATION2", "consultantAlert-viewList");
        var code = "";
        code = this.consDetail.CONSULTANT_NAME;
        var remarkEntry = "(CA)This entry has been returned by GM for " + code;
        ...
        var notiTitle = "This entry has been returned by General Manager";
        data.append("NOTI_TITLE", notiTitle);
        data.append("CA_COMMENT", this.model.comment);

        try {
          const response = await servicesModule3.updateGmApproval(data);
          //alert(JSON.stringify(response));
    ...
    rejectCase: async function () {
      if (confirm("Are you sure you want to reject this entry?")) {
        ...
        data.append("CREATE_BY", JSON.parse(user).user_id);
          ...
          //reject direct to 1st level and email to both 1st & 2nd level
        const approvalLevel = await services06Module0.getApprovalLevel(6, 1);
        data.append("APPR_LIST", JSON.stringify(approvalLevel));
        ...
        // }
        code = this.consDetail.CONSULTANT_NAME;
        var remarkEntry = "(CA)This entry has been rejected by GM for " + code;
        ...
        var notiTitle = "This entry has been rejected by General Manager";
        data.append("NOTI_TITLE", notiTitle);
        data.append("CA_COMMENT", this.model.comment);
