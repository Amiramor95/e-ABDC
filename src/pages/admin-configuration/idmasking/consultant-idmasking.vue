...
<template>
  <va-card>
      ...
      <div class="header">EDIT CONSULTANT ID MASKING<i class="close icon">&times;</i></div>
      <div class="content">
        <vue-form-generator
          :model="model"
    ...
    <h4 slot="header" class="card-title"> <i class="fa fa-bank"></i>&nbsp;&nbsp;CONSULTANT ID MASKING SETTING</h4>
    <br />
    <div>
      <!-- <div class="content"> -->
      <div>
        ...
        <va-icon name="fa fa-search" slot="prepend" />
          ...
          <br />
          <h4>LIST OF CONSULTANT ID MASKING</h4>
...
export default {
  ...
  methods: {
   ...
   back() {
    ...
    const response = await servicesModule0.getConsultantAllMasking();
     console.log("Masking=", response);
    ...
    this.Masking = this.MaskingList.length;
     ...
     getLatestMasking: async function() {
       // let LatestMasking=0;
        try {
        const response = await servicesModule0.getLatestMasking("CONSULTANT");
        console.log("response=",response);
        if(response){
            this.LatestMasking=response.RUN_NO;
            this.model.CURRENT_RUN_NO= this.LatestMasking;
        }
        else{
            //this.LatestMasking=0;
            this.model.CURRENT_RUN_NO= this.LatestMasking;
    ...
    async addMasking () {
        if (this.$refs.Tab1Form.validate()) {
          ...
          //  data.append('CURRENT_RUN_NO', this.model.CURRENT_RUN_NO)
            data.append('PREFIX',  this.model.PREFIX)
            data.append('DESCRIPTION', this.model.DESCRIPTION)
            data.append('STATUS', this.model.STATUS)
             data.append('MASKING_TYPE', 'CONSULTANT')
            try {
              // eslint-disable-next-line no-unused-vars
              const response = await servicesModule0.createConsultantIdMasking(data)
              this.getAllMasking();
               this.getLatestMasking();
              this.model.RUN_NO="";
    ...
    const response = await servicesModule0.getConsultantMaskingById(data.CONSULTANT_MASKING_ID)
    this.$modals.simpleModalEditGender.$show()
    this.CONSULTANT_MASKING_ID = response.CONSULTANT_MASKING_ID
