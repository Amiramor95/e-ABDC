...
<template>
  <va-card>
      ...
      <div class="header">EDIT TRAINING PROVIDER ID MASKING<i class="close icon">&times;</i></div>
      <div class="content">
        <vue-form-generator
          :model="model"
    ...
    <h4 slot="header" class="card-title"> <i class="fa fa-bank"></i>&nbsp;&nbsp;TRAINING PROVIDER ID MASKING SETTING</h4>
    <br />
    <div>
      <!-- <div class="content"> -->
      <div>
        ...
        <va-icon name="fa fa-search" slot="prepend" />
          ...
          <br />
          <h4>LIST OF TRAINING PROVIDER ID MASKING</h4>
...
export default {
  ...
  methods: {
   ...
   back() {
    ...
    const response = await servicesModule0.getAllMasking();
    ...
    this.Masking = this.MaskingList.length;
     ...
     getLatestMasking: async function() {
       // let LatestMasking=0;
        try {
        const response = await servicesModule0.getLatestMasking("TPROVIDER");
    ...
    async addMasking () {
        if (this.$refs.Tab1Form.validate()) {
           ...
           // data.append('CURRENT_RUN_NO', this.model.CURRENT_RUN_NO)
            ...
            // data.append('MASKING_TYPE', 'TPROVIDER')
            try {
              // eslint-disable-next-line no-unused-vars
              const response = await servicesModule0.createIdMasking(data)
              this.getAllMasking();
               this.getLatestMasking();
              this.model.RUN_NO="";
    ...
    this.MASKING_ID = response.MASKING_ID
    ...
    async updateMaskingAPI () {
      if (this.$refs.TabEditForm.validate()) {
        try {
          const data = new FormData()
          data.append('MASKING_ID', this.MASKING_ID)
          ...
          const response = await servicesModule0.updateMasking(data)
          this.getAllMasking()
        } catch (error) {
          console.log(error)
        }
