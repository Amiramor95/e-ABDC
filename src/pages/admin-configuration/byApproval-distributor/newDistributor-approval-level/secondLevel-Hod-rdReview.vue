      ...
      <div class="container">
        <div class="card-body">
          <div class="row justify-content-center">
            <div class="col-lg-12">
              <va-card>
                <h4 slot="header" class="card-title">SECOND LEVEL HOD REGISTRATION DEPARTMENT REVIEW</h4>
                <div class="ml-3">
                    <vue-form-generator
                      tag="div"
                      :schema="schema"
...
export default {
  ...
  methods: {
    ...
    getAllApproval: async function() {
      const response = await servicesModule0.getAllApprovalList(1, "HOD REGISTRATION DEPARTEMENT APPROVAL");
    // this.approvalList = response;
      ...
      this.model.GROUP_NAME = response.COMBINE;
    ...
    //     data.append("APPR_LEVEL_NAME", "FIRST LEVEL REGISTRATION DEPARTMENT REVIEW");
    //     try {
     ...
     async submit() {
      if (this.$refs.RegForm.validate()) {
       ...
       data.append("APPROVAL_LEVEL_ID", this.APPROVAL_LEVEL_ID);
        data.append("APPR_GROUP_ID", this.model.GROUP_NAME.MANAGE_GROUP_ID);
        data.append("APPR_LEVEL_NAME", "HOD REGISTRATION DEPARTEMENT APPROVAL");
        data.append("APPR_PROCESSFLOW_ID", 1);
        data.append("APPR_INDEX", this.model.INDEX);
        data.append(
          "APPR_AUTO_APPROVAL_DAYS",
          this.model.AUTO_APPROVAL_DAYS
