    ...
    <div class="row justify-content-center">
     ...
     <div class="text-center text-danger my-2 align-center" v-if="!isDataReady">
      ...
      <va-card class=" ml-4 col-lg-10 my-1 px-1">
        <h5>
          <i class="va-icon app-sidebar-link__item-icon vuestic-iconset vuestic-iconset-dashboard"></i>&nbsp;&nbsp; <b>DISTRIBUTOR Dashboard Setting for EXTERNAL USER</b>
        </h5>
        <hr />
        <div class="ml-5">
            ...
            <table class="va-table">
                ...
                <!-- <td class="displaye_size_small">Small</td> -->
                    <!-- <td class="displaye_size_medium" v-if="typeof distributorSetting.setting_setup === 'object'">
                      <select name="SETTING_CHART_ID" v-model='distributorSetting.setting_setup["DISPLAY_SETTING_STYLE"]' :id="k" class="form-control control chart_box">
            ...
            fields: [
              {
                ...
                selectOptions: {
                  multiple: false,
                  key: "MANAGE_DISTRIBUTOR_GROUP_ID",
                  label: "GROUP_NAME",
                  searchable: true,
                },
                values: (model, schema) => {
  ...
  methods: {
    ...
    this.$router.push("/fimm/distributor_main_setting");
     ...
     getGroup: async function () {
      const response = await servicesModule0.getAllDistributorGroup();
      ...
      if(event.target.checked) {
       ...
       // console.log("Checklist1=",this.distributorSettingList);
        ...
        if (totalcheck <= 5) {
         ...
         if(id != '')
         {
         const response = await servicesModule0.deleteFmsDashboardSetting(id);
          this.getDistributorSetting();
         }
    ...
    getDistributorSetting: async function() {
       console.log("UserGroup=",this.userdata);
          const userID=this.userdata.user_id;
        const userTYPE=this.userdata.user_type;
        const typeID=1;
         const accessuserGROUP=this.model.groupSelection.MANAGE_DISTRIBUTOR_GROUP_ID;
        const accessuserDivision= 0;
        const accessuserDepartment=0;
         if(typeof accessuserGROUP != 'undefined')
          {
              console.log("External=");
        const response = await servicesModule0.getDistributorSettingList(userID,userTYPE,typeID,accessuserDivision,accessuserDepartment,accessuserGROUP);
        console.log("CPD REsponse=",response);
   ...
   async submit() {
    ...
    var f3 = this.$refs.Tab3Form.validate();
      ...
      if(allCheckedSetting <=6)
      {
        try {
              // eslint-disable-next-line no-unused-vars
            const response = await services06Module1.createDistributorDashboardSetting(checklist,userID,userTYPE,userGROUP,accessuserDivision,accessuserDepartment,accessuserGROUP);
            this.getDistributorSetting();
