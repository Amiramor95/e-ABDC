...
<template>
  <va-card>
      ...
      <div class="header">EDIT ANNUAL FEE DASHBOARD SETTING<i class="close icon">&times;</i></div>
      <div class="content">
        <vue-form-generator
          :model="model"
    ...
    <h4 slot="header" class="card-title"> <i class="va-icon app-sidebar-link__item-icon vuestic-iconset vuestic-iconset-dashboard"></i>&nbsp;&nbsp;ANNUAL FEE DASHBOARD STATISTICS</h4>
    <br />
    <div>
      <!-- <div class="content"> -->
      <div>
        <div class="ml-5">
          ...
          <br />
          <br />
          <h4>LIST OF ANNUAL FEE DASHBOARD</h4>
...
export default {
  ...
  data () {
    return {
      ...
      distributorList: null,
       subtypeList:[
        {'type_id' : 1,'usergroup': 'RD'},
        {'type_id' : 2,'usergroup': 'R&A'},
        {'type_id' : 3,'usergroup': 'FIN'}
                  ],
      model: {
        // TAB 1
        DASHBOARD_MAIN_ID: '',
            ...
            fields: [
              {
              ...
              },
               {
                type: "vueMultiSelect",
                inputType: "Text",
                model: "editList.DASHBOARD_SUB_TYPE",
                label: "USER GROUP",
                placeholder: "SELECT Type",
                selectOptions: {
                  multiple: false,
  ...
  methods: {
   ...
   back() {
      this.$router.push("/fimm/afms-dashboard-main-setting");
    },
    //search Acc Code
    ...
    const response = await servicesModule0.getDistributorDashboardSetting("ANNUAL");
    ...
    async addStatistic () {
        if (this.$refs.Tab1Form.validate()) {
            ...
            data.append('DASHBOARD_DESCRIPTION', this.model.DASHBOARD_DESCRIPTION)
             data.append('DASHBOARD_TYPE', 'ANNUAL')
            data.append('DASHBOARD_SUB_TYPE', this.model.DASHBOARD_SUB_TYPE.type_id)
            try {
              // eslint-disable-next-line no-unused-vars
    ...
    if(response.DASHBOARD_SUB_TYPE == 1)
    {
      tp = "RD";
    }
    if(response.DASHBOARD_SUB_TYPE == 2)
    {
      tp = "R&A";
    }
     if(response.DASHBOARD_SUB_TYPE == 3)
    {
      tp = "FIN";
    }
    ...
    this.model.editList = response
     this.model.editList.DASHBOARD_SUB_TYPE = {
          type_id: response.DASHBOARD_SUB_TYPE,
          usergroup: tp,
      };
  //  this.model.editList.DASHBOARD_SUB_TYPE.type_id = response.DASHBOARD_SUB_TYPE
    },
      ...
      if (this.$refs.TabEditForm.validate()) {
        try {
            ...
            data.append('DASHBOARD_LIST', this.model.editList.DASHBOARD_LIST)
             data.append('DASHBOARD_SUB_TYPE', this.model.editList.DASHBOARD_SUB_TYPE.type_id)
