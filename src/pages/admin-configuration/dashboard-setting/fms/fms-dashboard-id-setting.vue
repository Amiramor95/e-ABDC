    ...
    <div class="row justify-content-center">
     ...
     <div class="text-center text-danger my-2 align-center" v-if="!isDataReady">
      ...
      <va-card class=" ml-4 col-lg-10 my-1 px-1">
        <h5>
          <i class="va-icon app-sidebar-link__item-icon vuestic-iconset vuestic-iconset-dashboard"></i>&nbsp;&nbsp; <b>FMS R&A and ID Dashboard Setting</b>
        </h5>
        <hr />
        <div class="ml-5">
...
export default {
  ...
  methods: {
    back() {
    this.$router.push("/fimm/fund-dashboard-main-setting");
    },
    ...
    getFMSSetting: async function() {
       //console.log("UserGroup=",this.userdata);
        ...
        if(typeof accessuserDivision != 'undefined' && typeof accessuserDepartment != 'undefined' && typeof accessuserGROUP != 'undefined')
          {
        const response = await servicesModule0.getFMSSettingList(userID,userTYPE,userGROUP,accessuserDepartment,typeID,accessuserDivision,accessuserGROUP);
   ...
   async submit() {
     var f3 = this.$refs.Tab3Form.validate();
      ...
      if(allCheckedSetting <=6)
      {
        try {
              // eslint-disable-next-line no-unused-vars
            const response = await servicesModule0.createFmsDashboardSetting(checklist,userID,userTYPE,userGROUP,typeID,accessuserDivision,accessuserDepartment,accessuserGROUP);
            this.getFMSSetting();
