    ...
    <div class="row justify-content-center">
     ...
     <div class="text-center text-danger my-2 align-center" v-if="!isDataReady">
      ...
      <va-card class=" ml-4 col-lg-10 my-1 px-1" >
        <h5>
          <i class="va-icon app-sidebar-link__item-icon vuestic-iconset vuestic-iconset-dashboard"></i>&nbsp;&nbsp; <b>CPD Dashboard Setting</b>
        </h5>
        <hr />
        <div class="ml-5">
...
export default {
  ...
  methods: {
    back() {
    this.$router.push("/fimm/cpd-dashboard-main-setting");
    },
      ...
      if(event.target.checked) {
       ...
       // console.log("Checklist1=",this.distributorSettingList);
        ...
        if (totalcheck <= 5) {
         ...
         if(id != '')
         {
         const response = await servicesModule0.deleteCpdDashboardSetting(id);
          this.getCPDSetting();
         }
       ...
       // console.log("UserTYPE=",this.userdata.user_type);
        ...
        const accessuserRole=this.model.authorizationSelection.AUTHORIZATION_LEVEL_ID;
         if(typeof accessuserDivision != 'undefined' && typeof accessuserDepartment != 'undefined' && typeof accessuserGROUP != 'undefined')
          {
        const response = await servicesModule0.getCPDSettingList(userID,userTYPE,accessuserDivision,accessuserDepartment,accessuserGROUP);
    ...
    if(f3){
        ...
        const accessuserDepartment=this.model.departmentSelection.MANAGE_DEPARTMENT_ID;
       // const accessuserRole=this.model.authorizationSelection.AUTHORIZATION_LEVEL_ID;
        const response = await servicesModule0.getCPDSettingList(userID,userTYPE,userGROUP,accessuserDivision,accessuserDepartment,accessuserGROUP);
   ...
   async submit() {
      ...
      if(allCheckedSetting <=6)
      {
        try {
              // eslint-disable-next-line no-unused-vars
            const response = await servicesModule0.createCpdDashboardSetting(checklist,userID,userTYPE,accessuserDivision,accessuserDepartment,accessuserGROUP);
            this.getCPDSetting();
