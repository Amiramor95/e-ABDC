...
const TABS = [
  { title: "ADD ACCESS", value: "tab1" },
  { title: "VIEW ACCESS", value: "tab2" },
];
export default {
  ...
  methods: {
    ...
    getCONSULTANTSetting: async function() {
       console.log("UserTYPE=",this.userdata.user_type);
        ...
        {
        const response = await servicesModule0.getConsultantSettingList(userID,userTYPE,userGROUP,accessuserDivision,accessuserDepartment,accessuserGROUP);
        console.log("accessuserDivision=",accessuserDivision);
         console.log("accessuserDepartment=",accessuserDepartment);
  ...
  async searchRecord(){
   ...
   async submit() {
      ...
      //console.log("formContents=",allCheckedSetting);
      ...
      if(allCheckedSetting <=6 )
      {
        try {
              // eslint-disable-next-line no-unused-vars
            const response = await servicesModule0.createConsultantDashboardSetting(checklist,userID,userTYPE,userGROUP,accessuserDivision,accessuserDepartment,accessuserGROUP);
            this.getCONSULTANTSetting();
