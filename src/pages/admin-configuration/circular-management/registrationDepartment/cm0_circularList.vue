<template>
  <va-card>
    ...
    <div v-else >
        ...
        <div class="row">
          <div class="col">
            <h4 class="card-title">LIST OF {{ this.$route.params.DPMT_NAME }} DEPARTMENT CIRCULAR </h4>
          </div>
      ...
      >
        ...
        <template slot="actions" slot-scope="props">
          <h4 class="mt-2">
            ...
            ></span>
            <span
              v-if="props.rowData.PUBLISH_STATUS == 0 || props.rowData.PUBLISH_STATUS == 3 || props.rowData.PUBLISH_STATUS == 5"
              v-b-tooltip.hover
              title="edit"
...
export default {
  ...
  mounted() {
    this.getAllEvent();
    this.getTaskStatus();
    console.log('Department Id Param Value',this.$route.params.MANAGE_DEPARTMENT_ID);
  },
  methods: {
    back() {
      this.$router.go(-1);
    ...
    async getAllEvent() {
      ...
      if(dept){
        const response = await servicesModule0.getAllCircular(dept);
        this.items = response;
        this.eventListCount = this.items.length;
        this.isDataReady = true;
      }else{
        this.$router.push("/fimm/circular-setting");
      }
   ...
   search: debounce(function (term) {
    ...
    async searchCircular() {
      ...
      var dept = this.$route.params.MANAGE_DEPARTMENT_ID; 
      const response = await servicesModule0.searchCircular(dept,title,status);
      this.items = response;
      this.eventListCount = this.items.length;
