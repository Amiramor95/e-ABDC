...
<template>
  <va-card>
      ...
      <div class="header">Edit Salutation<i class="close icon">&times;</i></div>
      <div class="content">
        <vue-form-generator
          :model="model"
          :schema="editSalutationSchema"
          :options="formOptions"
          ref="TabEditForm"
        />
      </div>
    ...
    <h4 slot="header" class="card-title">SALUTATION SETTING</h4>
    <br />
    <div>
      <tabs
        :tabs="tabs"
      ...
      >
        <va-icon name="fa fa-search" slot="prepend" />
          ...
          <br />
          <h4>LIST OF SALUTATION</h4>
          <!-- list -->
          <va-card class="mt-2">
            <va-data-table
              :fields="Salutationfields"
              :data="filteredDataSalutation"
              :per-page="parseInt(perPage)"
              :datacount="Salutation"
            ...
            clickable>
              ...
              <template slot="actions" slot-scope="props">
                <h4 class="mt-2">
                    ...
                    v-on:click="editSalutation(props.rowData)"
                  >
                    <i class="fa fa-edit"></i
                  ></span>
                  <span class="badge badge-danger mr-1" v-on:click="deleteSalutation(props.rowData)">
...
export default {
  ...
  computed: {
    ...
    filteredDataSalutation() {
      if (!this.term || this.term.length < 1) {
        return this.SalutationList
      }

      return this.SalutationList.filter((item) => {
        return this.term
          .toLowerCase()
          .split(" ")
          .every(
            ...
            fields: [
              {
                type: "input",
                inputType: "Text",
                model: "editSalutation.SET_PARAM",
                label: "Salutation",
                required: true,
                validator: "required",
                placeholder: "Enter Salutation",
  ...
  methods: {
   ...
   back() {
    ...
    getAllSalutation: async function() {
    const response = await servicesModule0.getAllSettingGeneralList("SALUTATION");
    this.SalutationList = response;
    this.Salutation = this.SalutationList.length;
    },
    ...
    if (this.$refs.Tab1Form.validate()) {
	  ...
	  data.append('SET_TYPE', 'SALUTATION')
      try {
        // eslint-disable-next-line no-unused-vars
    ...
    this.model.editSalutation = response
    },

    async updateSalutationAPI () {
      if (this.$refs.Tab1Form.validate()) {
        try {
        const data = new FormData()
        data.append('SETTING_GENERAL_ID', this.SETTING_GENERAL_ID)
        data.append('SET_PARAM', this.model.editSalutation.SET_PARAM)
