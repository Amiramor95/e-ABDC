...
<template>
  <va-card>
      ...
      <div class="header">Edit Registration Declaration<i class="close icon">&times;</i></div>
      <div class="content">
        <vue-form-generator
          :model="model"
    ...
    <h5>
          <i class="fa fa-bank"></i>&nbsp;&nbsp; <b>Registration Declaration</b>
        </h5>
        <hr />
    <div>
      <tabs
...
export default {
  ...
  data () {
    return {
      ...
      model: {
        // TAB 1
        DECLARATION_SET_TYPE: 'registration',
        DECLARATION_SET_PARAM: 'DISTRIBUTOR',
  ...
  methods: {
    ...
    back() {
      this.$router.push("/fimm/consultant_declaration");
    },
    ...
    const response = await servicesModule0.getAllDistributorRegistrationDeclaration("CONSULTANT","registration");
    ...
    async addRegDec () {
        if (this.$refs.Tab1Form.validate()) {
            ...
            data.append('DECLARATION_SET_TYPE', 'registration')
            data.append('DECLARATION_SET_PARAM', 'CONSULTANT')
            try {
              // eslint-disable-next-line no-unused-vars
