  ...
  mounted() {
   ...
   data() {
        return {
          ...
          approvalLevelList: [
        {
          id: "1", name: "FIMM HEAD OF DEPARTMENT", predecessor:"", autoApprovalDay:"", autoRejectDay:"", toggleStatus:0, datatoggle:false, compType: [
     ...
     methods: {
        ...
        onToggleChange(item,event){
          ...
          async submit(item){
            ...
            const data = new FormData();
                ...
                data.append('APPR_PROCESSFLOW_NAME','ANNOUCEMENT MANAGEMENT - L(1)(j)');
              try {
                // eslint-disable-next-line no-unused-vars
                const response = await servicesModule0.createApprovalList(data);
                  this.getAllApproval('ANNOUCEMENT MANAGEMENT - L(1)(j)');
      ...
      getAllApproval: async function() {
          const response = await servicesModule0.getAllApprovalList('ANNOUCEMENT MANAGEMENT - L(1)(j)');
          this.approvalLevelList = response;
