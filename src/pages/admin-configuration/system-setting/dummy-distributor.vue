...
<template>
  <va-card>
      <vudal name="simpleModalEditDummy" class="widthModal">
      <div class="header">Edit Dummy Distributor<i class="close icon">&times;</i></div>
      <div class="content">
        <vue-form-generator
          :model="model"
          :schema="editDummySchema"
          :options="formOptions"
           ref="TabEditForm"
        />
      </div>
    ...
    <h4 slot="header" class="card-title"> <i class="fa fa-bank"></i>&nbsp;&nbsp;Dummy Distributor Management</h4>
    <br />
    <div>
      <!-- <div class="content"> -->
      <div>
        ...
        <va-icon name="fa fa-search" slot="prepend" />
          ...
          <br />
          <h4>LIST OF DUMMY DISTRIBUTORS</h4>
          <!-- list -->
          <div class="float-right">
              <va-input
              :value="term"
...
console.log("term=",this.term);
        ...
        groups: [
          {
            ...
            fields: [
               ...
               type: "vueMultiSelect",
                inputType: "Text",
                model: "searchDummyDist.DIST_TYPE",
                label: "Application Type",
                placeholder: "Enter Type",
                selectOptions: {
                  multiple: false,
  ...
  methods: {
    ...
    getAllDistributorType: async function () {
      const response = await servicesModule0.getDummyDistributorType();
    ...
    async addDummyDistributor() {
        if (this.$refs.Tab1Form.validate()) {
          console.log('this.model.DIST_TYPE =', this.model.DIST_TYPE);
            const data = new FormData()
            data.append('DIST_NAME', this.model.DIST_NAME)
            data.append('DIST_TYPE', this.model.DIST_TYPE.DISTRIBUTOR_TYPE_ID)
            try {
              // eslint-disable-next-line no-unused-vars
              const response = await servicesModule0.createDummyDistributor(data)
              this.getAllDummyDistributor()
