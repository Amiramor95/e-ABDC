          ...
          <div class="ml-5">
            ...
            <div v-for="dept in departmentList">
              <div class="row">
                ...
                <div class="col"> 
                  <toggle-button
                    v-model="IS_ACTIVE[dept.MANAGE_DEPARTMENT_ID]"                 
                    :labels="{ checked: 'ON', uncheked: 'What now?' }"
                    @change="onToggleChange(dept.MANAGE_DEPARTMENT_ID)"
                  />
                </div>
                <div class="col">
                <button
                    v-if="dept.CIRCULAR_STATUS == 1"
  ...
  methods: {
    ...
    async onToggleChange(dept){
      if (confirm("Are you sure you want to update ?")) {
        ...
        const response = await servicesModule04.setCircularStatus(data);
        //alert(dept);
      }
      this.getAllDepartment();
    },
    // Get all Depaartment
    getAllDepartment1: async function () {
    ...
    getAllDepartment: async function () {
      ...
      const response = await servicesModule04.getCircularDept();
      response.forEach((item) => {
        this.IS_ACTIVE[item.MANAGE_DEPARTMENT_ID] = (item.CIRCULAR_STATUS == 1) ? true : false;
      });
      this.departmentList = response;
      //this.DepartmentCount = this.departmentList.length;
      this.isDataReady = true; // Kill Loader
    ...
    LRA() {
      this.$router.push("/fimm/lra-circular-List");
    },
