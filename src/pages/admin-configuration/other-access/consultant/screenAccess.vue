...
<template>
  ...
  <loder v-show="isLoader" />
    ...
    <vudal name="viewScreen" class="widthModal">
      <div class="header">
        View Consultant Access Screen<i class="close icon">&times;</i>
      </div>
      <div class="">
        <table class="table">
          <tbody>
            <tr>
              ...
              <td scope="col">{{ model.editScreen.CONSULTANT_MANAGE_GROUP_ID.CONSULTANT_GROUP_NAME }}</td>
            </tr>
            <tr>
              <td scope="col" class="font-weight-bold">Authorization: </td>
              <td scope="col">{{ model.editScreen.CONSULTANT_AUTHORISATION_LEVEL_ID.AUTHORIZATION_LEVEL_NAME }}</td>
            </tr>
          </tbody>
        </table>
...
export default {
  ...
  watch : {
    ...
    //   if(val) {
    //       this.getSubmoduleByModule(val.CONSULTANT_MANAGE_MODULE_ID);
    //       this.getAllScreens();
    //   }else{
    //     this.getAllScreens();
    //   }
  ...
  methods: {
    ...
    checkScreen(val,data){
      ...
      if(event.target.checked){
          ...
          screenArr = screenArr.filter(function( obj ) {
            return obj.CONSULTANT_MANAGE_SCREEN_ID !== data.CONSULTANT_MANAGE_SCREEN_ID;
          });
      ...
      if (f2) {
        ...
        this.screenListSave.forEach((item) => {
          if (item.selected == true) {
            this.checkboxArray.push(item.CONSULTANT_MANAGE_SCREEN_ID);
          }
        });
        // alert(this.checkboxArray)
        const data = new FormData();
        data.append("CONSULTANT_MANAGE_GROUP_ID", this.model.CONSULTANT_MANAGE_GROUP_ID.CONSULTANT_MANAGE_GROUP_ID);
        data.append("CONSULTANT_AUTHORISATION_LEVEL_ID", this.model.CONSULTANT_AUTHORISATION_LEVEL_ID.AUTHORIZATION_LEVEL_ID);
        data.append("CONSULTANT_SCREEN_ID", JSON.stringify(this.checkboxArray));
        data.append("CONSULTANT_USER_ID", this.model.USER.USER_ID);
        try {
          const response = await servicesModule0.createConsScreenAccess(data);
          if(response != 'error'){
            this.screenAccessReset();
            this.getAuthorizationPage();
          }
    ...
    screenAccessReset(){
      this.model.CONSULTANT_MANAGE_GROUP_ID = '';
      this.model.CONSULTANT_AUTHORISATION_LEVEL_ID = '';
      this.model.USER = '';
    ...
    getAuthorizationPage: async function() {
      const response = await servicesModule0.getAllConsAuthPageList();
      this.AuthPageList = response;
      this.screenAccessListCount = this.AuthPageList.length;
      this.isDataReady = true; // Kill Componel Loader
    },
    ...
    getScreenByModule: async function() {
      const response = await servicesModule0.getConsManageScreenByModule(
        this.model.moduleSelection.CONSULTANT_MANAGE_MODULE_ID,
        this.model.submoduleSelection.CONSULTANT_MANAGE_SUBMODULE_ID
      );
      ...
      arrData.forEach((item) => {  
         arr.push(item.CONSULTANT_MANAGE_SCREEN_ID);
      });
      screenData.forEach((item) => {  
        if(arr.includes(item.CONSULTANT_MANAGE_SCREEN_ID)){ 
          item.selected = true;
        }
      });
    ...
    getScreenByModuleEdit: async function() {
      const response = await servicesModule0.getConsManageScreenByModule(
        this.model.editScreen.moduleSelection.CONSULTANT_MANAGE_MODULE_ID,
        this.model.editScreen.submoduleSelection.CONSULTANT_MANAGE_SUBMODULE_ID
      );
      var screenData = response;
       // for selected exist
      var arrData = this.screenListSave;
    ...
    onModuleData: async function (newVal, schema) { //alert(schema);  
      ...
      if (schema == "moduleSelection" || schema == "editScreen.moduleSelection" && newVal) {
        this.model.submoduleSelection = "";
        const response = await servicesModule0.getConsManageSubModuleByModule(newVal.CONSULTANT_MANAGE_MODULE_ID);
    ...
    //   const response = await servicesModule0.getConsManageSubModuleByModule(data);
    //   this.submoduleList = response;
    //   this.submoduleListCount = this.submoduleList.length;
    // },
    ...
    async editAuthPage(data) {
      this.isLoader = true; // Loder 
      const response = await servicesModule0.getConsScreenAuthPageById(
        data.CONSULTANT_SCREEN_ACCESS_ID
      );
      this.CONSULTANT_SCREEN_ACCESS_ID = data.CONSULTANT_SCREEN_ACCESS_ID;
    ...
    setEditData(response){
      ...
      this.model.editScreen.submoduleSelection = "";
       ...
       this.model.editScreen.USER = {
        ...
        //   this.model.editScreen.moduleSelection = {
        //     CONSULTANT_MANAGE_MODULE_ID: response.allScreenByModule[0].CONSULTANT_MANAGE_MODULE_ID,
        //     CONSULTANT_MOD_NAME: response.allScreenByModule[0].CONSULTANT_MOD_NAME
        //   }
        //   this.model.editScreen.submoduleSelection = {
        //     CONSULTANT_MANAGE_SUBMODULE_ID: response.allScreenByModule[0].CONSULTANT_MANAGE_SUBMODULE_ID,
        //     CONSULTANT_SUBMODULE_NAME: response.allScreenByModule[0].CONSULTANT_SUBMODULE_NAME
        //   }
        // } 
       this.model.editScreen.CONSULTANT_MANAGE_GROUP_ID = {
         CONSULTANT_MANAGE_GROUP_ID: response.ScreenAccess.CONSULTANT_MANAGE_GROUP_ID,
         CONSULTANT_GROUP_NAME: response.ScreenAccess.CONSULTANT_GROUP_NAME
       }
       this.model.editScreen.CONSULTANT_AUTHORISATION_LEVEL_ID = {
       ...
       // for selected 
        ...
        screenData.forEach((item) => {  
          ...
          if(arrData.includes(item.CONSULTANT_MANAGE_SCREEN_ID)){ 
            item.selected = true;
          }
        });
    ...
    async updateAuthPageAPI() {
      try {
        ...
        // alert(this.checkboxArray)
        data.append("CONSULTANT_SCREEN_ACCESS_ID",this.CONSULTANT_SCREEN_ACCESS_ID);
        data.append("CONSULTANT_MANAGE_GROUP_ID", this.model.editScreen.CONSULTANT_MANAGE_GROUP_ID.CONSULTANT_MANAGE_GROUP_ID);
        data.append("CONSULTANT_AUTHORISATION_LEVEL_ID", this.model.editScreen.CONSULTANT_AUTHORISATION_LEVEL_ID.AUTHORIZATION_LEVEL_ID);
        data.append("CONSULTANT_SCREEN_ID", JSON.stringify(this.checkboxArray));
        data.append("CONSULTANT_USER_ID", this.model.editScreen.USER.USER_ID);
        const response = await servicesModule0.updateConsManageScreenAccess(data);
        this.getAuthorizationPage();
        this.isLoader = false; // Kill Loder 
      } catch (error) {
        //console.log(error);
    ...
    async deleteScreen(data){
      ...
      if (confirm("Are you sure you want to delete?")){
        this.isLoader = true; // Loder 
        const response = await servicesModule0.deleteConsultantAccess(data.CONSULTANT_SCREEN_ACCESS_ID);
        this.getAuthorizationPage();
        this.isLoader = false; // Kill Loder 
      }
    }
  ...
  data() {
    return {
      ...
      model: {
        ...
        CONSULTANT_SCREEN_ID: '',
        CONSULTANT_MANAGE_GROUP_ID: '',
        CONSULTANT_AUTHORISATION_LEVEL_ID: '',
        USER: '',
        ALL_ACCESS:false,
            ...
            fields: [
              {
                label: "Group Name",
                type: "vueMultiSelect",
                model: "CONSULTANT_MANAGE_GROUP_ID",
                placeholder: "Select Group",
                selectOptions: {
                  multiple: false,
                  key: "CONSULTANT_MANAGE_GROUP_ID",
                  label: "CONSULTANT_GROUP_NAME",
                  searchable: true
                },
              ...
              },
              {
                ...
                type: "vueMultiSelect",
                model: "CONSULTANT_AUTHORISATION_LEVEL_ID",
            ...
            fields: [
              {
                label: "Group Name",
                type: "vueMultiSelect",
                model: "editScreen.CONSULTANT_MANAGE_GROUP_ID",
                placeholder: "Select Group",
                selectOptions: {
                  multiple: false,
                  key: "CONSULTANT_MANAGE_GROUP_ID",
              ...
              },
              {
                label: "Authorization Level Name",
                type: "vueMultiSelect",
                model: "editScreen.CONSULTANT_AUTHORISATION_LEVEL_ID",
                placeholder: "Select Authorisation",
                selectOptions: {
                  multiple: false,
                  key: "AUTHORIZATION_LEVEL_ID",
