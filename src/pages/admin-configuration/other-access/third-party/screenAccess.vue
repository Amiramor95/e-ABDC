        ...
        <table class="table">
          <tbody>
            <tr>
              ...
              <td scope="col">{{ model.editScreen.THIRDPARTY_MANAGE_GROUP_ID.THIRDPARTY_GROUP_NAME }}</td>
            </tr>
            <tr>
              <td scope="col" class="font-weight-bold">Authorization: </td>
              <td scope="col">{{ model.editScreen.THIRDPARTY_AUTHORISATION_LEVEL_ID.AUTHORIZATION_LEVEL_NAME }}</td>
            </tr>
          </tbody>
        </table>
...
export default {
  ...
  methods: {
    ...
    checkScreen(val,data){
      ...
      if(event.target.checked){
          ...
          screenArr = screenArr.filter(function( obj ) {
            return obj.THIRDPARTY_MANAGE_SCREEN_ID !== data.THIRDPARTY_MANAGE_SCREEN_ID;
          });
      ...
      if (f2) {
        ...
        this.screenListSave.forEach((item) => {
          if (item.selected == true) {
            this.checkboxArray.push(item.THIRDPARTY_MANAGE_SCREEN_ID);
          }
        });
        // alert(this.checkboxArray)
        const data = new FormData();
        data.append("THIRDPARTY_MANAGE_GROUP_ID", this.model.THIRDPARTY_MANAGE_GROUP_ID.THIRDPARTY_MANAGE_GROUP_ID);
        data.append("THIRDPARTY_AUTHORISATION_LEVEL_ID", this.model.THIRDPARTY_AUTHORISATION_LEVEL_ID.AUTHORIZATION_LEVEL_ID);
        data.append("THIRDPARTY_SCREEN_ID", JSON.stringify(this.checkboxArray));
        data.append("THIRDPARTY_USER_ID", this.model.USER.USER_ID);
        try {
          const response = await servicesModule0.createThirdScreenAccess(data);
          if(response != 'error'){
            this.dataReset();
            this.getAuthorizationPage();
          }
    ...
    dataReset(){
      this.model.THIRDPARTY_MANAGE_GROUP_ID = '';
      this.model.THIRDPARTY_AUTHORISATION_LEVEL_ID = '';
      this.model.USER = '';
    ...
    getAuthorizationPage: async function() {
      const response = await servicesModule0.getAllThirdAuthPageList();
    ...
    getScreenByModule: async function() {
      const response = await servicesModule0.getThirManageScreenByModule(
        this.model.moduleSelection.THIRDPARTY_MANAGE_MODULE_ID,
        this.model.moduleSelection.THIRDPARTY_SUBMODULE_ID,
      );
      ...
      arrData.forEach((item) => {  
         arr.push(item.THIRDPARTY_MANAGE_SCREEN_ID);
      });
      screenData.forEach((item) => {  
        if(arr.includes(item.THIRDPARTY_MANAGE_SCREEN_ID)){ 
          item.selected = true;
        }
      });
    ...
    getScreenByModuleEdit: async function() {
      const response = await servicesModule0.getThirManageScreenByModule(
        this.model.editScreen.moduleSelection.THIRDPARTY_MANAGE_MODULE_ID,
        this.model.editScreen.moduleSelection.THIRDPARTY_SUBMODULE_ID,
      );
      var screenData = response;
       // for selected exist
      var arrData = this.screenListSave;
    ...
    async editAuthPage(data) {
      this.isLoader = true; // Loder 
      const response = await servicesModule0.getThirdScreenAuthPageById(
        data.THIRDPARTY_SCREEN_ACCESS_ID
      );
      this.THIRDPARTY_SCREEN_ACCESS_ID = data.THIRDPARTY_SCREEN_ACCESS_ID;
    ...
    setEditData(response){
      ...
      this.model.editScreen.submoduleSelection = "";
       ...
       this.model.editScreen.USER = {
        ...
        //   this.model.editScreen.moduleSelection = {
        //     THIRDPARTY_MANAGE_MODULE_ID: response.allScreenByModule[0].THIRDPARTY_MANAGE_MODULE_ID,
        //     THIRDPARTY_MOD_NAME: response.allScreenByModule[0].THIRDPARTY_MOD_NAME
        //   }
        //   this.model.editScreen.submoduleSelection = {
        //     THIRDPARTY_SUBMODULE_ID: response.allScreenByModule[0].THIRDPARTY_SUBMODULE_ID,
        //     THIRDPARTY_SUBMOD_NAME: response.allScreenByModule[0].THIRDPARTY_SUBMOD_NAME
        //   }
        // } 
       this.model.editScreen.THIRDPARTY_MANAGE_GROUP_ID = {
         THIRDPARTY_MANAGE_GROUP_ID : response.ScreenAccess.THIRDPARTY_MANAGE_GROUP_ID,
         THIRDPARTY_GROUP_NAME : response.ScreenAccess.THIRDPARTY_GROUP_NAME 
       }
       this.model.editScreen.THIRDPARTY_AUTHORISATION_LEVEL_ID = {
      ...
      this.model.editScreen.ALL_ACCESS = response.allAccess;
        ...
        screenData.forEach((item) => {  
          ...
          if(arrData.includes(item.THIRDPARTY_MANAGE_SCREEN_ID)){ 
            item.selected = true;
          }
        });
    ...
    async updateAuthPageAPI() {
      try {
        ...
        data.append(
          "THIRDPARTY_SCREEN_ACCESS_ID",
          this.THIRDPARTY_SCREEN_ACCESS_ID
        );
        data.append("THIRDPARTY_MANAGE_GROUP_ID", this.model.editScreen.THIRDPARTY_MANAGE_GROUP_ID.THIRDPARTY_MANAGE_GROUP_ID);
        data.append("THIRDPARTY_AUTHORISATION_LEVEL_ID", this.model.editScreen.THIRDPARTY_AUTHORISATION_LEVEL_ID.AUTHORIZATION_LEVEL_ID);
        data.append("THIRDPARTY_SCREEN_ID", JSON.stringify(this.checkboxArray));
        data.append("THIRDPARTY_USER_ID", this.model.editScreen.USER.USER_ID);
        const response = await servicesModule0.updateThirdManageScreenAccess(data);
        this.getAuthorizationPage();
        this.isLoader = false; // Kill Loder 
      } catch (error) {
        console.log(error);
    ...
    async deleteScreen(data){
      ...
      if (confirm("Are you sure you want to delete?")){
        this.isLoader = true; // Loder 
        const response = await servicesModule0.deleteThirdpartyAccess(data.THIRDPARTY_SCREEN_ACCESS_ID);
        this.getAuthorizationPage();
        this.isLoader = false; // Kill Loder 
      }
    },
  ...
  data() {
    return {
      ...
      model: {
        ...
        THIRDPARTY_SCREEN_ID: '',
        THIRDPARTY_MANAGE_GROUP_ID: '',
        THIRDPARTY_AUTHORISATION_LEVEL_ID: '',
        USER: '',
        ALL_ACCESS:false,
            ...
            fields: [
              {
                label: "Group Name",
                type: "vueMultiSelect",
                model: "THIRDPARTY_MANAGE_GROUP_ID",
                placeholder: "Select Group",
                selectOptions: {
                  multiple: false,
                  key: "THIRDPARTY_MANAGE_GROUP_ID",
                  label: "THIRDPARTY_GROUP_NAME",
                  searchable: true
                },
              ...
              },
              {
                ...
                type: "vueMultiSelect",
                model: "THIRDPARTY_AUTHORISATION_LEVEL_ID",
            ...
            fields: [
              {
                label: "Group Name",
                type: "vueMultiSelect",
                model: "editScreen.THIRDPARTY_MANAGE_GROUP_ID",
                placeholder: "Select Group",
                selectOptions: {
                  multiple: false,
                  key: "THIRDPARTY_MANAGE_GROUP_ID",
              ...
              },
              {
                label: "Authorization Level Name",
                type: "vueMultiSelect",
                model: "editScreen.THIRDPARTY_AUTHORISATION_LEVEL_ID",
                placeholder: "Select Authorisation",
                selectOptions: {
                  multiple: false,
                  key: "AUTHORIZATION_LEVEL_ID",
