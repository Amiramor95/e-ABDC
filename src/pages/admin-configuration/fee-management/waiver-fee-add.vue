<template>
  ...
  <loder v-show="isLoader" />
    ...
    <div>
      ...
      <vudal name="simpleModalEditWaiverFee" class="widthModal">
        <div class="header">
          EDIT WAIVER FEE<i class="close icon">&times;</i>
        </div>
        <div class="">
          <vue-form-generator
            :model="model"
...
async submit() {
    ...
    var f5 = this.$refs.Tab5Form.validate();
      if (f1 && f2 && f3 && f4 && f5) {
        ...
        data.append("WAIVER_FEE_TYPE_ID", this.model.WAIVER_FEE_TYPE_ID.FEE_ID);
        data.append("WAIVER_TYPE_ID", this.model.WAIVER_TYPE_ID.WAIVER_ID);
        ...
        data.append("WAIVER_START_DATE",  moment(this.model.WAIVER_START_DATE).format("YYYY-MM-DD"));
        data.append("WAIVER_END_DATE",  moment(this.model.WAIVER_END_DATE).format("YYYY-MM-DD"));
        ...
        try {
          //console.log(data);
          const response = await servicesModule0.createWaiverFee(data);
          this.resetFormData();
         // this.getAllWaiverFee();
          this.$router.push(
        "/fimm/waiver-fee"
