...
<div class="float-right">
        ...
        <span
                v-b-tooltip.hover
                title ="New Waiver Fee"
                v-on:click="waiver_fee_add"
...
<div class="row">
      ...
      <div class="ml-5">
        ...
        <br />
        <h4> List of Waiver Fee </h4>
        <br/>
        <!-- list -->
        <va-card class="mt-2">
          <va-data-table
            :fields="WaiverFeeListing"
            :data="filteredWaiverFee"
            :per-page="parseInt(perPage)"
            :datacount="WaiverFeeCount"
            ...
            <template slot="actions" slot-scope="props">
              <h4 class="mt-2">
                <span
                  ...
                  v-on:click="editWaiverFee(props.rowData)"
                ...
                <span
                  ...
                  v-on:click="deleteWaiverFee(props.rowData)"
                >
...
export default {
  ...
  methods: {
    back() {
      ...
      waiver_fee_add() {
    this.$router.push("/fimm/waiver-fee-add");
    },
    dateValidation(type){
...
async search() {
       ...
       if(eDate == "")
      {
        ...
        try {
          //console.log(data);
          const response = await servicesModule0.searchWaiverFee(data);
           response.forEach(item => {
        item.EXAM_FEE = this.format_number(item.EXAM_FEE);
        item.PROCESSING_FEE = this.format_number(item.PROCESSING_FEE)
        item.ANNUAL_FEE = this.format_number(item.ANNUAL_FEE)
    ...
    async editWaiverFee(data) {
       this.$router.push({
        ...
        params: { WAIVER_FEE_ID: data.WAIVER_FEE_ID }
      });
