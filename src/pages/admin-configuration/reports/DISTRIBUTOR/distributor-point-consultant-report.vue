...
export default {
 ...
 props: ["id"],
  ...
  methods: {
    back() {
      this.$router.push("/fimm/distributor-point-report");
    },
    getCasUtcTaggingListReport: async function() {
      ...
      setTimeout(()=>{
        ...
        var encodedID = btoa(userID+'/'+userTYPE+'/'+pointID);
        this.legacySystemHTML = `<iframe id="report1" src="`+Vue.config.KoolServerURL+`distributorpointconsultantreport?q=`+ encodedID +`" width="100%" ></iframe>`;
