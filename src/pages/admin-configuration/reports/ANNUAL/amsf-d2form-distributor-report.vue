...
<template>
  <va-card>
    <h4 slot="header" class="card-title">
      ...
      >&nbsp;&nbsp;Form D2 *External Distributor (IUTA,CPRA,CUTA,IPRA)
    </h4>
    <br />
    <div>
      <div>
...
export default {
  mounted() {
    ...
    this.getCasUtcTaggingListReport();
     console.log("DISTRIBUTOR=",this.userdata);
  },
  components: {
    Tabs,
    Vudal,
  ...
  methods: {
    back() {
      this.$router.push("/fimm/annual-report-main-setting");
    },
    getCasUtcTaggingListReport: async function() {
        const userID=this.userdata.user_id;
        const userTYPE=this.userdata.user_type;
        const userDistID= 399;//this.userdata.USER_DIST_ID;
       setTimeout(()=>{
         this.isDataReady = true;
       }, 10000);
         var encodedID = btoa(userID+'/'+userTYPE+'/'+userDistID);
         this.legacySystemHTML = `<iframe id="report1" src="`+Vue.config.KoolServerURL+`amsfd2formdistributorreport?q=`+ encodedID +`" width="100%" ></iframe>`;
