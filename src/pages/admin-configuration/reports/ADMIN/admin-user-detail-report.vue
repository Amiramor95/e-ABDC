...
export default {
  ...
  methods: {
    back() {
      this.$router.push("/fimm/admin-user-report");
    },
    getCasUtcTaggingListReport: async function() {
        const userID=this.userdata.user_id;
        const userTYPE=this.userdata.user_type;
        const uID=this.$route.params.userid;
        const tID=this.$route.params.companyid;
      setTimeout(()=>{
        this.isDataReady = true;
      }, 20000);
        var encodedID = btoa(userID+'/'+userTYPE+'/'+uID+'/'+tID);
        this.legacySystemHTML = `<iframe id="report1" src="`+Vue.config.KoolServerURL+`adminuserdetailreport?q=`+ encodedID +`" width="100%" ></iframe>`;
