<template>
  <va-card>
    ...
    <div v-else>
      ...
      <div class="row">
      <div class="col-md-6">
        <b><h2 class="card-title">Edit Letter Template</h2></b>
      </div>
      <div class="col-md-6">
      ...
      <div class="row">
        ...
        <div class="col-md-9">
          ...
          <input
            type="text"
            v-model="LETTER.CAS_LETTER_TITTLE"
      ...
      <div class="ml-2 float-right">
        ...
        >
          <i class="fa fa-paper-plane" /> &nbsp; Update
        </b-button>
      </div>
    </div>  
  </va-card>
...
export default {
  ...
  mounted () {
    var data = localStorage.getItem('edit-letter')
    this.LETTER = JSON.parse(data)
    this.isDataReady = true; // Kill component loader

  },
  methods: {
    async getLetterById () {
      console.log('id letter : ' + this.model.CAS_LETTER_ID)
      const response = await servicesModule0.getLetterById(
        this.model.CAS_LETTER_ID,)
        //console.log("filee : ");
      this.model = response.letter; 
    ...
    async submit () {
      ...
      data.append('CAS_LETTER_ID', this.LETTER.CAS_LETTER_ID)
      data.append("CAS_LETTER_TITTLE", this.LETTER.CAS_LETTER_TITTLE)
      data.append("CAS_LETTER_DESC", this.LETTER.CAS_LETTER_DESC)
      try {
        // eslint-disable-next-line no-unused-vars
        const response = await servicesModule0.updateLetter(data);
