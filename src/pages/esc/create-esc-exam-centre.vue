...
export default {
  ...
  methods: {
    ...
    navigateToList () {
      this.$router.push({ name: 'list-esc-centre-management' })
    },
    validateCentreForm () {
    ...
    onModelUpdated (newVal, schema) {
      ...
      if (schema === 'CENTRE_MODE') {
        ...
        if (this.model.CENTRE_COUNTRY.SET_CODE === 'MY') {
          this.getCity(this.model.CENTRE_POSTAL.SETTING_CITY_ID)
        }
    ...
    async submit () {
      ...
      if (response && confirm('Are you sure you want to proceed this submission ?')) {
        ...
        const data = JSON.parse(localStorage.getItem('CREATE-ESC-EXAM-CENTRE_COMPANY_ID'))
        data1.append('COMPANY_ID', data)
