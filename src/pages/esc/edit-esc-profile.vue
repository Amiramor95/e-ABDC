...
<template>
  <va-card>
    ...
    <vue-form-generator
      :model="model"
      :options="formOptions"
      :schema="companyUserProfileSchema"
...
export default {
  ...
  mounted () {
    ...
    //   element.style.display = 'none'
    // })
    this.profileData = JSON.parse(localStorage.getItem('edit-esc-profile-provider'))
  ...
  data () {
    return {
      ...
      citizenship: [],
      filters: {
        COMPANY_STATUS: { TS_ID: 3, name: 'APPROVED' },
        APPROVAL_STATUS: { TS_ID: 22, name: 'ACTIVE' },
      },
      model: {
        // company admin name
        FULL_NAME: '',
  ...
  methods: {
    ...
    async getAllCompanies () {
      ...
      // )
      this.company = await companyManagementServices.listAllCompany()
      this.model.COMPANY_NAME = this.company.find(x => x.COMPANY_PROFILE_ID === this.profileData.COMPANY_ID)
    },
    async getProfileDetail () {
      this.model.FULL_NAME = this.profileData.FULL_NAME
      this.model.CITIZENSHIP = this.profileData.CITIZENSHIP
      if (this.profileData.CITIZENSHIP === 286) {
        this.model.ESC_NRIC = this.profileData.NRIC
      } else {
        this.model.ESC_PASSPORT_NO = this.profileData.PASSPORT_NO
        this.model.ESC_PASSPORT_EXPIRY_NO = this.profileData.PASSPORT_EXPIRY_NO
      }
      this.model.EMAIL = this.profileData.EMAIL
      this.model.PHONE_NO = this.profileData.PHONENO
    ...
    showRequestTacNumberEmailIfChanged () {
      return (this.profileData.EMAIL !== this.model.EMAIL)
    },
    showRequestTacNumberMobileIfChanged () {
      return (this.profileData.PHONENO !== this.model.PHONE_NO)
    },
