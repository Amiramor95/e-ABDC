        ...
        >
          ...
          <template slot="actions" slot-scope="props">
            <h5 class="mt-1">
              <span
                v-b-tooltip.hover
                title="Edit Esc Profile"
                v-on:click="editRecord(props.rowData)"
                v-if="(props.rowData.APPROVAL_STATUS === taskStatusConstants.REJECTED_DISPLAY)"
...
export default {
  ...
  methods: {
    // create
    create () {
      this.$router.push({ name: 'exam-type-creation' })
    },
    // search
    ...
    getAllRecord: async function () {
      ...
      const response = await examCentreProviderManagementServices.listAllProfile(
        {
    ...
    async editRecord (data) {
      localStorage.setItem('edit-esc-profile-provider', JSON.stringify(data))
      this.$router.push({
        name: 'edit-esc-profile-provider',
      })
    },
