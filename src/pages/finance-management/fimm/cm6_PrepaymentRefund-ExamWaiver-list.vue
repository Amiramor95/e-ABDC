<template>
  <va-card>
    <template slot="header">
      ...
      <div class="row">
        <div class="col">
          <h4 class="card-title">List of Pre-Payment Exam Waiver Refund</h4>
        </div>
        <div v-if="!loading" class="col float-right">
          <div class="float-right">
            <h5>
              ...
              <vue-json-to-csv :json-data="csv"
                               :csv-title="'Pre-Payment Refund Exam Waiver'"
                  ...
                  DEBIT_AMOUNT: { title: 'Transaction Amount' },
                  ...
                  AMOUNTINBASE: {
                    title: 'Transaction Amount In Base Currency (Tax Incl)',
                  },
                  BANKAMOUNT: { title: 'Bank Amount' },
                  INDUSTRYCODE: { title: 'Standard Industry Code' }, }"
              >
                <b-badge v-b-tooltip.hover title="Export as CSV for Biztrak" class="mr-1" variant="success">
    ...
    >
      ...
      <template slot="actions" slot-scope="props">
        <h4 class="mt-2">
          ...
          >
              <i class="fa fa-eye" v-if="props.rowData.STATUS === 'PENDING'"></i>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async getAllList () {
        this.loading = true
        const response = await servicesModule6.getAllRefundExamWaiver()
        this.items = response
      ...
      async getcsv () {
        const response = await servicesModule6.getCSV5()
        this.csv = response
      },

      async getEXcel () {
        const res = await servicesModule6.refund_exam_waiverExcel()
      ...
      async viewRecord (data) {
        localStorage.setItem('PrepaymentRefund-ExamWaiver-Details', JSON.stringify(data))
        this.$router.push({
      ...
      EXCEL_DATA () {
        return  this.filteredData.map(x => {
          return {
            ...
            DIST_TYPE_NAME:x.DIST_TYPE_NAME,
            TOTAL_CONS:x.TOTAL_CONS,
            TOTAL_AMOUNT:x.DEBIT_AMOUNT,
            STATUS:x.STATUS
          }
        })
      },
    ...
    data () {
      return {
        ...
        excelname: 'download-prepaymentExamWaiverRefund',
        sheetname: 'prepaymentExamWaiverRefund',
        dateFormat: '',
        term: null,
        perPage: '10',
