<template>
  <va-card>
    <template slot="header">
      ...
      <div class="row">
        <div class="col">
          <h4 class="card-title">List of Pre-Payment Refund Distributor Termination</h4>
        </div>
        <div class="col float-right">
          <div class="float-right">
            <h5>
              ...
              ></downloadExcel>
              <vue-json-to-csv
                :json-data="csv" :csv-title="'Pre-Payment Refund Distributor Termination'"
                @success="val => handleSuccess(val)"
                @error="val => handleError(val)"
                :labels="{REFNO3: { title: 'Reference Number 3' },
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async getAllList () {
        this.items = await servicesModule6.getRefundDistributorTerminationList()
        this.ListCount = this.items.length
      },

      async getcsv () {
        this.csv = await servicesModule6.getCSV4()
      },
      ...
      async viewRecord (data) {
        localStorage.setItem('PrepaymentRefund-DistributorTermination-Details', JSON.stringify(data))
        this.$router.push({
      ...
      async filterRecord () {
        ...
        try {
          const response = await servicesModule6.getRefundDistributorTerminationList(
      ...
      EXCEL_DATA(){
        return this.filteredData.map(x => {
            return {
              ...
              TXN_NUMBER :x.TRANSACTION_NUMBER,
              DIST_NAME:x.DIST_NAME,
              DIST_TYPE_NAME:x.DIST_TYPE_NAME,
              TOTAL_CONSULTANT:x.TOTAL_CONS,
    ...
    data () {
      return {
        ...
        excelname: 'download-DistributorTerminationList',
        sheetname: 'DistributorTerminationPayment',
        dateFormat: '',
        term: null,
        perPage: '10',
