...
export default {
  mounted() {
    var data = localStorage.getItem("t-preVettingViewProgramList");
  ...
  methods: {
    ...
    async returnRecord(progData) {
      if (confirm("Are you sure to request return of this submission?")) {
        ...
        data.append("APPR_LIST2", JSON.stringify(approvalLevel2));
          ...
          //notification
      data.append("NOTI_LOCATION", "apprPreVettingTrpList");
    ...
    async viewRecord(data) {
      localStorage.setItem(
        "t-preVettingViewProgramDetails",
        JSON.stringify(data)
      );
      this.$router.push({
    ...
    async amendRecord(data) {
      localStorage.setItem("t-preVettingAmendProgram", JSON.stringify(data));
      this.$router.push({
        name: "t-preVettingAmendProgram",
      });
    },
    async editRecord(data) {
      localStorage.setItem("t-preVettingEditProgram", JSON.stringify(data));
      this.$router.push({
