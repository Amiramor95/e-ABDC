<template>
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">List of Post-Vetting Programs</h4>
      <br />
...
export default {
  ...
  methods: {
    ...
    getAllRecord: async function () {
      const response = await servicesModule4.getProgramById(0, 1, 1);
      this.RecordList = response;
      this.RecordCount = this.RecordList.length;
    },
    async viewRecord(data) {
      localStorage.setItem(
        "f-postVettingViewProgramList",
        JSON.stringify(data)
      );
      this.$router.push({
        name: "f-postVettingViewProgramList",
      });
    },

    async addProgram(data) {
      localStorage.setItem(
        "f-postVettingRepeateSubmission",
        JSON.stringify(data)
      );
      this.$router.push({
        name: "f-postVettingRepeateSubmission",
      });
    },
  },
