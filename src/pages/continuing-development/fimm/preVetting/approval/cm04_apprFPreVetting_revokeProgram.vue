  ...
  <div class="row">
    <div class="col-12">
      <template>
        ...
        <va-card v-else>
          <div class="row">
            <h4 slot="header" class="card-title">FPre-Vetting : Revoke Program</h4>
          </div>
          <br />
...
export default {
  mounted() {
    var data = localStorage.getItem("approval-FpreVetting-revokeProgram");
  ...
  methods: {
    ...
    revokeCpd: async function () {
      if (confirm("Are you sure to revoke this program?")) {
        ...
        //notification
        data.append("NOTI_LOCATION", "apprFPreVettingDistributorList");
        var remarkEntry = "(CPD-PRE)Program for "+ this.PDATA.PROG_CODE +" has been revoked by FIMM";
        data.append("NOTI_REMARK", remarkEntry);
        data.append("DIST_CREATE_BY", this.PROGDATA.DIST_CREATE_BY);
        data.append("FDIST_CREATE_BY",true);
        ...
        try {
          ...
          const response = await servicesModule4.revokeProgram(data);
