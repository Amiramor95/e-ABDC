...
export default {
  ...
  methods: {
    ...
    async editSlo(data,rowID) {
      ...
      // this.model.hours-=parseInt(data.HOURS)
    ...
    submit: async function () {
      if (confirm("Are you sure you want to submit this entry?")) {
        ...
        var code = "";
        var remarkEntry =
          "(CPD-PRE)New entry pending approval for FIMM Training Provider";
    ...
    async upload() {
      ...
      fileReader.onload = async function (e) {
        ...
        if (this.model.hours != response1.totalHours) {
          if (
            confirm(
              "Total Hours entered is not matched with total hours in SLO. System will auto update the field total hours."
            )
          ) {
