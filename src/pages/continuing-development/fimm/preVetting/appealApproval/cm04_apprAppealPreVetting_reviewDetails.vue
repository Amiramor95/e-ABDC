  ...
  <va-card v-else>
    <div class="row">
      <h4 slot="header" class="card-title">
        FPre-Vetting : Review Appeal Program
      </h4>
    </div>
    <br />
...
export default {
  mounted() {
    var data = localStorage.getItem("approvalAppeal-preVetting-reviewDetails");
  ...
  methods: {
    ...
    getAppealDocumentById: async function () {
      const response = await servicesModule4.getAppealDocumentById(
        this.PROGDATA.PROG_DETAILS_ID,1
      );
      //alert(JSON.stringify(response));
      response.forEach((element) => {
        console.log(element.DOC_BLOB);
    ...
    saveAsDraft: async function () {
      if (confirm("Are you sure you want to save as draft?")) {
        ...
        data.append("PROG_APPEAL_APPR_ID", this.PROGDATA.PROG_APPEAL_APPR_ID);
        ...
        try {
          const response = await servicesModule4.updateAppealApproval(data);
          const response1 = await servicesModule4.uploadRevokeDocument(data);
    ...
    revokeCpd: async function () {
      if (confirm("Are you sure to revoke this program?")) {
        ...
        //notification
        data.append("NOTI_LOCATION", "f-preVettingProgramList");
        var remarkEntry = "(CPD-PRE)Your appeal submission has been declined by FIMM";
    ...
    approve: async function () {
      if (confirm("Are you sure you want to approve this program?")) {
        ...
        //notification
        data.append("NOTI_LOCATION", "f-preVettingProgramList");
        var remarkEntry = "(CPD-PRE)This program has been approved by FIMM";
        data.append("NOTI_REMARK", remarkEntry);
        data.append("DIST_CREATE_BY", this.PROGDATA.DIST_CREATE_BY);
        data.append("DISTRIBUTOR_ID", this.PROGDATA.COMPANY_ID);
        data.append("POINTS",this.PDATA.PROG_HOURS);
    ...
    viewParticipant: async function () {
      ...
      localStorage.setItem(
        "approval-FpreVetting-view-participant",
        this.PROGDATA.PROG_DETAILS_ID
      );
      this.$router.push({
            ...
            fields: [
              {
                ...
                text: "Choose a File",
                model: "fileNew",
                type: "vfg-custom-multiple-file-edit-appeal-approvalCpd",
                styleClasses: "col-md-12",
