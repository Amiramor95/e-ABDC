  ...
  <va-card v-else>
    <div class="row">
      <h4 slot="header" class="card-title">
        FReading : List of Training Providers
      </h4>
    </div>
...
export default {
  ...
  methods: {
    ...
    getRecordByDist: async function () {
      const response = await services07Module4.getModuleByTRPReading();
    ...
    async viewRecord(data) {
      localStorage.setItem("approvalFReadingModuleList", JSON.stringify(data));
      this.$router.push({
    ...
    async viewProgram(data) {
      localStorage.setItem("approval-f-reading-allProgram", JSON.stringify(data));
      this.$router.push({
