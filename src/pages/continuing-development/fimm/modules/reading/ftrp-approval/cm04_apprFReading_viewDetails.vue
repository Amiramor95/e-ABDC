<template>
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">FReading : View Program Details</h4>
    </div>
...
export default {
  mounted() {
    var data = localStorage.getItem("f-approvalReadingViewDetails");
  ...
  methods: {
    ...
    async back() {
      this.$router.push('/fimm/approvalFReadingModuleList');
    },
      ...
      if (r == true) {
        ...
        try {
          ...
          if (response.status) {
            ...
            this.$router.push('/fimm/approvalFReadingDistributorList');
          }
        } catch (error) {
          this.submitApproval = false;
        }
    ...
    approveCpd: async function () {
      if (confirm("Are you sure to approve of this submission?")) {
        ...
        var remarkEntry = "(CPD-FReading)This entry has been approved by FIMM";
        data.append("NOTI_REMARK", remarkEntry);
        data.append("APPR_REMARK", this.model.comment);
        data.append("NOTI_LOCATION", "f-readingModuleList");
        const user = localStorage.getItem("user");
    ...
    returnCpd: async function () {
      if (confirm("Are you sure to return of this submission?")) {
        ...
        data.append("APPR_REMARK", this.model.comment);
        var remarkEntry = "(CPD-Feading)This Entry has been returned by FIMM";
        data.append("NOTI_REMARK", remarkEntry);
        data.append("NOTI_LOCATION", "f-readingModuleList");
        ...
        const approvalLevel2 = await services06Module0.getApprovalLevel(28, 2);
        data.append("APPR_LIST2", JSON.stringify(approvalLevel2));
        try {
    ...
    rejectCpd: async function () {
      if (confirm("Are you sure to reject this submission?")) {
        ...
        data.append("APPR_PUBLISH_STATUS", 1);
        data.append("APPR_REMARK", this.model.comment);
        var remarkEntry = "(CPD-FReading)This Entry has been rejected by FIMM";
        data.append("NOTI_REMARK", remarkEntry);
        data.append("NOTI_LOCATION", "f-readingModuleList");
        var noted = "REVIEWED BY " + JSON.parse(user).USER_GROUP_NAME;
        data.append("NOTE", noted);
