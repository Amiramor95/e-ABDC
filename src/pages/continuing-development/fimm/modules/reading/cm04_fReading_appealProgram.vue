<template>
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">FReading : Appeal Program</h4>
    </div>
    <br />
    <va-card>
...
export default {
  mounted() {
    var data = localStorage.getItem("f-readingAppealProgram");
  ...
  methods: {
    ...
    accept: async function () {
      if (
        ...
        try {
          const response = await servicesModule4.appealReadingProgram(data);
          this.$router.go(-1);
    ...
    appeal: async function () {
      if (confirm("Are you sure you want to submit appeal?")) {
        ...
        //notification
        data.append("NOTI_LOCATION", "approvalFReadingDistributorList");
        ...
        try {
          const response = await servicesModule4.appealReadingProgram(data);
          const response1 = await servicesModule4.uploadReadingAppealDocument(data);
    ...
    viewParticipant: async function () {
      const data = new FormData();
      data.append("PROG_DETAILS_ID", this.PROGDATA.MODULE_ID);

      localStorage.setItem(
        "approval-readingView-participant",
        this.PROGDATA.MODULE_ID
      );
      this.$router.push({
