  ...
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">
        FReading : Review Appeal Program
      </h4>
    </div>
    <br />
    <va-card>
...
<script>
  ...
  export default {
    mounted() {
      var data = localStorage.getItem("appealFReadingViewDetails");
    ...
    methods: {
      ...
      getRevokeDocumentById: async function () {
        const response = await servicesModule4.getReadingRevokeDocumentById(
      ...
      getAppealDocumentById: async function () {
        const response = await servicesModule4.getReadingAppealDocumentById(
      ...
      saveAsDraft: async function () {
        if (confirm("Are you sure you want to save as draft?")) {
          ...
          data.append("READING_APPEAL_APPR_ID", this.PROGDATA.READING_APPEAL_APPR_ID);
          ...
          try {
            const response = await servicesModule4.updateReadingModuleAppealApproval(data);
      ...
      revokeCpd: async function () {
        if (confirm("Are you sure to revoke this program?")) {
          ...
          //notification
          data.append("NOTI_LOCATION", "f-appealReadingListProgramList");
          var remarkEntry = "(CPD-FReading)Your appeal submission has been declined by FIMM";
      ...
      approve: async function () {
        if (confirm("Are you sure you want to approve this program?")) {
          ...
          //notification
          data.append("NOTI_LOCATION", "f-appealReadingListProgramList");
          var remarkEntry = "(CPD-FReading)This program has been approved by FIMM";
          data.append("NOTI_REMARK", remarkEntry);
          data.append("DIST_CREATE_BY", this.PROGDATA.DIST_CREATE_BY);
          data.append("DISTRIBUTOR_ID", this.PROGDATA.COMPANY_ID);
