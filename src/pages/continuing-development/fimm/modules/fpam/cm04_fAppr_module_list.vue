...
export default {
  ...
  methods: {
    ...
    back() {
      this.$router.push('/fimm/approval-FP-allProgram');
    }, // back button
    ...
    getAllRecord: async function () {
      ...
      //   );
      const response =
        await services07Module4.getModuleApprovaledByCompanyNModuleType(
          this.PROGDATA.COMPANY_ID,
          5,
          this.$route.query.fpCategory // FP_CATEGORY
        );
    ...
    async revokeRecord(data) {
      localStorage.setItem(
        "approval-fCertified-revokeProgram",
        JSON.stringify(data)
      );
      this.$router.push({
