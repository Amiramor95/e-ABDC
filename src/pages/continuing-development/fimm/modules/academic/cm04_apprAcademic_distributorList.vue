  ...
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">
        Academic/Professional Qualification : List of Distributors
      </h4>
    </div>
...
export default {
  ...
  methods: {
    ...
    getRecordByDist: async function () {
      const response = await services07Module4.getModuleByDistributor(4);
    ...
    async viewRecord(data) {
      localStorage.setItem("approvalAcademicModuleList", JSON.stringify(data));
      this.$router.push({
    ...
    async viewProgram(data) {
      localStorage.setItem("approval-academic-allProgram", JSON.stringify(data));
      this.$router.push({
        ...
        return this.term
          ...
          .every(
            (v) =>
              ...
              // item.TOTAL_NEW.toLowerCase().includes(v) ||
              // item.TOTAL_REQUESTRETURN.toLowerCase().includes(v)||
              // item.TOTAL_DRAFT.toLowerCase().includes(v)||
              // item.TOTAL_RETURN.toLowerCase().includes(v)
          );
      });
    },
  },
