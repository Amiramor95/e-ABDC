<template>
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">Academic : Revoke Program</h4>
    </div>
    <br />
...
<script>
  ...
  export default {
    mounted() {
      var data = localStorage.getItem("approval-academic-revokeProgram");
    ...
    methods: {
      ...
      revokeCpd: async function () {
        if (confirm("Are you sure to revoke this program?")) {
          ...
          var remarkEntry = "(CPD-ACADEMIC)Program for "+ this.PROGDATA.MODULE_CODE +" has been revoked by FIMM";
          ...
          try {
            ...
            const response = await servicesModule4.revokeQualificationProgram(data);
