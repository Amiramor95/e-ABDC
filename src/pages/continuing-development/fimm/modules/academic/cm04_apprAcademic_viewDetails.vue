<template>
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">Academic/Professional Qualification : View Program Details</h4>
    </div>
...
export default {
  mounted() {
    var data = localStorage.getItem("approvalAcademicViewDetails");
  ...
  methods: {
    ...
    async back() {
      this.$router.push('/fimm/approvalAcademicModuleList');
    },
    ...
    approveCpd: async function () {
      if (confirm("Are you sure to approve of this submission?")) {
        ...
        var remarkEntry = "(CPD-Academic)This entry has been approved by FIMM";
        data.append("NOTI_REMARK", remarkEntry);
        data.append("APPR_REMARK", this.model.comment);
        data.append("NOTI_LOCATION", "d-academicModuleList");
        const user = localStorage.getItem("user");
    ...
    returnCpd: async function () {
      if (confirm("Are you sure to return of this submission?")) {
        ...
        data.append("APPR_REMARK", this.model.comment);
        var remarkEntry = "(CPD-Academic)This Entry has been returned by FIMM";
        data.append("NOTI_REMARK", remarkEntry);
        data.append("NOTI_LOCATION", "d-academicModuleList");
    ...
    rejectCpd: async function () {
      if (confirm("Are you sure to reject this submission?")) {
        ...
        data.append("APPR_PUBLISH_STATUS", 1);
        data.append("APPR_REMARK", this.model.comment);
        var remarkEntry = "(CPD-Academic)This Entry has been rejected by FIMM";
        data.append("NOTI_REMARK", remarkEntry);
        data.append("NOTI_LOCATION", "d-academicModuleList");
        var noted = "REVIEWED BY " + JSON.parse(user).USER_GROUP_NAME;
        data.append("NOTE", noted);
