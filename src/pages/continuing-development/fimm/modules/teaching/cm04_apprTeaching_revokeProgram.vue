<template>
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">Teaching : Revoke Program</h4>
    </div>
    <br />
...
<script>
  ...
  export default {
    mounted() {
      var data = localStorage.getItem("approval-teaching-revokeProgram");
    ...
    methods: {
      ...
      revokeCpd: async function () {
        if (confirm("Are you sure to revoke this program?")) {
          ...
          var remarkEntry = "(CPD-TEACHING)Program for "+ this.PROGDATA.MODULE_CODE +" has been revoked by FIMM";
          ...
          try {
            ...
            const response = await servicesModule4.revokeTeachingProgram(data);
