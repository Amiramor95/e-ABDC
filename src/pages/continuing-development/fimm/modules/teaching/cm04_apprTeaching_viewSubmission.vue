...
<script>
  ...
  export default {
    mounted() {
      var data = localStorage.getItem("approval-teaching-viewSubmission");
    ...
    methods: {
      ...
      async back() {
        this.$router.push('/fimm/approval-teaching-allProgram');
      },
      ...
      getDocumentById: async function (moduleId) {
        const response = await services07Module4.getOneTeachingDocument(moduleId);
