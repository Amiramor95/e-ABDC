<template>
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">Writing : Revoke Program</h4>
    </div>
    <br />
    <va-card>
...
export default {
  mounted() {
    var data = localStorage.getItem("approval-writing-revokeProgram");
  ...
  methods: {
    ...
    revokeCpd: async function () {
      if (confirm("Are you sure to revoke this program?")) {
        ...
        var remarkEntry = "(CPD-WRITING)Program for "+ this.PROGDATA.MODULE_CODE +" has been revoked by FIMM";
        ...
        try {
          ...
          const response = await servicesModule4.revokeWritingProgram(data);
          // const response1 = await servicesModule4.uploadWritingRevokeDocument(data);
          //
          this.$router.push({
