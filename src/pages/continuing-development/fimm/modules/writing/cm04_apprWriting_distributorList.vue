  ...
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">
        Writing and Publishing Books/Articles : List of Distributors
      </h4>
    </div>
...
export default {
  ...
  methods: {
    ...
    getRecordByDist: async function () {
      const response = await services07Module4.getModuleByDistributor(1);
      this.RecordList = response;
      this.RecordCount = this.RecordList.length;
    },
    async viewRecord(data) {
      localStorage.setItem("approvalWritingModuleList", JSON.stringify(data));
      this.$router.push({
    ...
    async viewProgram(data) {
      localStorage.setItem("approval-writing-allProgram", JSON.stringify(data));
      this.$router.push({
