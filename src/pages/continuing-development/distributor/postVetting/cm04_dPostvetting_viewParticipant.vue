<template>
  <va-card>
          ...
          <div v-if="!this.isApproval">
            ...
            <div class="ml-1 col-md-12">
              ...
              <div class="float-right">
                ...
                <button
                  v-if="this.existed != 'RECORD EXISTED' && this.existed != 'RECORD NOT FOUND'"
    ...
    <div class="float-right" v-if="!this.isApproval">
      <button
        @click="submit"
        type="button"
        class="ml-1 btn btn-success btn-fill btn-md"
...
export default {
  props: {},
  mounted() {
    this.isApproval=localStorage.getItem("postVetting-approval");
  ...
  methods: {
    submit: async function () {
      ...
      if (confirm("Are you sure you want to submit?")) {
        ...
        data.append("PROG_DETAILS_ID",localStorage.getItem("d-postVettingViewParticipant"));

        const responsePart = await servicesModule4.getProgramParticipantByID(
          localStorage.getItem("d-postVettingViewParticipant")
        );
    ...
    //general
    back() {
      localStorage.removeItem('postVetting-approval')
      this.$router.go(-1);
    },
  },
  computed: {
