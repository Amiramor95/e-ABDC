      ...
      >
        ...
        <template slot="actions" slot-scope="props">
          <h5 class="mt-1">
            <span
              v-if="
                props.rowData.TOTAL_NEW != 0 ||
                props.rowData.TOTAL_REQUESTRETURN != 0 ||
                props.rowData.TOTAL_DRAFT != 0
              "
              v-b-tooltip.hover
  ...
  methods: {
    ...
    async viewRecord(data) {
      localStorage.setItem(
        "approval-postVetting-programList",
        JSON.stringify(data)
      );
      this.$router.push({
    ...
    async viewProgram(data) {
      localStorage.setItem(
        "approval-postVetting-allProgram",
        JSON.stringify(data)
      );
      this.$router.push({
    ...
    RecordFilteredData() {
      ...
      return this.RecordList.filter((item) => {
        return this.term
          ...
          .split(" ")
          .every(
            (v) => item.DIST_NAME.toLowerCase().includes(v)||
