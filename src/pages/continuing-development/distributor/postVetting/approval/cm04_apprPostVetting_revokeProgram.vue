<template>
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">Post-Vetting : Revoke Program</h4>
    </div>
    <br />
    <va-card>
...
export default {
  mounted() {
    var data = localStorage.getItem("approval-postVetting-revokeProgram");
  ...
  methods: {
    ...
    revokeCpd: async function () {
      if (confirm("Are you sure to revoke this program?")) {
        ...
        var remarkEntry = "(CPD-POST)Program for "+ this.PDATA.PROG_CODE +" has been revoked by FIMM";
