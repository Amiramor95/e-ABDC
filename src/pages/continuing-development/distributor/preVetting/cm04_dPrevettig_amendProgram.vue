<template>
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">Pre-Vetting : Amend Submission</h4>
    </div>
    <br />
    <va-card>
      <div class="ml-2 col-md-12">
        Program Details
<!--        <span v-if="visibleProgram == false"-->
<!--          >: <b>Proposed Start Date ( {{ this.model.startDate }} )</b></span-->
<!--        >-->
...
export default {
  props: {},
  mounted() {
    var data = localStorage.getItem("d-preVettingAmendProgram");
  ...
  methods: {
    ...
    async upload() {
      if (confirm("Are you sure to upload this file?")) {
        ...
        console.log(this.model.fileParticipant);
        data.append("PROG_DETAILS_ID", this.PROGDATA.PROG_DETAILS_ID);

        try {
          const response = await servicesModule4.importExcelPreVetting(data);
          //this.$router.go(-1);
          // this.model.fileParticipant=[];
          this.getAllParticipant();
          this.getRejectedList();
