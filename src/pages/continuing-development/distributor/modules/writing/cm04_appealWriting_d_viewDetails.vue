    ...
    <va-card>
      <div>
        <table class="table table-striped">
          ...
          <tr>
            <td style="width: 30%">Revoke Justification</td>
            <td style="width: 1%">:</td>
...
<script>
  ...
  export default {
    mounted() {
      var data = localStorage.getItem("d-appealWritingViewDetails");
    ...
    methods: {
      ...
      getAppealDocumentById: async function () {
        const response = await servicesModule4.getWritingAppealDocumentById(
      ...
      getApprovalDocumentById: async function () {
        const response = await servicesModule4.getWritingAppealApprovalDocumentById(
          this.PROGDATA.MODULE_ID
        );
        //alert(JSON.stringify(response));
        response.forEach((element) => {
      ...
      saveAsDraft: async function () {
        if (confirm("Are you sure you want to save as draft?")) {
          ...
          data.append("WRITING_APPEAL_APPR_ID", this.PROGDATA.WRITING_APPEAL_APPR_ID);
      ...
      revokeCpd: async function () {
        if (confirm("Are you sure to revoke this program?")) {
          ...
          var remarkEntry = "(CPD-WRITING)Your appeal submission has been declined by FIMM";
      ...
      approve: async function () {
        if (confirm("Are you sure you want to approve this program?")) {
          ...
          //notification
          data.append("NOTI_LOCATION", "d-writingModuleList");
          var remarkEntry = "(CPD-WRITING)This program has been approved by FIMM";
          data.append("NOTI_REMARK", remarkEntry);
          data.append("DIST_CREATE_BY", this.PROGDATA.DIST_CREATE_BY);
          data.append("DISTRIBUTOR_ID", this.PROGDATA.COMPANY_ID);
