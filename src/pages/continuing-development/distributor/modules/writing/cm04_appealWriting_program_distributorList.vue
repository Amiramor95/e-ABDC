  ...
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">
        Appeal List of Writing Submission Programs
      </h4>
    </div>
...
export default {
  mounted() {
    var data = localStorage.getItem("d-appealWritingListProgramList");
  ...
  methods: {
    ...
    back() {
      this.$router.push('/fimm/d-appealWritingList');
    }, // back button

    // getAllRecord
    getAllRecord: async function () {
      //alert(this.PROGDATA.COMPANY_ID);
      const response = await servicesModule4.getModuleByDistId(
    ...
    async editRecord(data) {
     //alert(JSON.stringify(data))
      localStorage.setItem("d-appealWritingViewDetails", JSON.stringify(data));
      this.$router.push({
