...
export default {
  ...
  mounted() {
    ...
    this.checkRunNo();
    this.realm_Name = JSON.parse(localStorage.getItem("realmName"));
  ...
  methods: {
    ...
    async uploadExcel() {
      ...
      var response = await this.$refs.vueFileAgent.upload(
        "/api/module4/writing_modules/upload?CREATE_BY=" +
          this.USER.user_id +
          "&COMPANY_ID=" +
    ...
    filter(value) {
      if (value.includes("-")) {
        this.term =
          value.substr(0, 6) + value.substr(7, 2) + value.substr(10, 4);
      } else {
        this.term = value;
      }
    },
    ...
    onBeforeDeleteSupportDoc: function (fileRecord, participant) {
      ...
      if (confim == true) {
        ...
        var i =
          this.fileRecordsForUploadSupportDoc[
            participant.MODULE_WRITING_ID
          ].indexOf(fileRecord);
        if (i !== -1) {
          // queued file, not yet uploaded. Just remove from the arrays
          this.fileRecordsForUploadSupportDoc[
  ...
  data() {
    return {
      submitLoading: false,
      title: "Writing and Publishing Books/Articles : New Submission",
      USER: null,
      realm_Name: null,
      USERID: null,
      COMPANYID: null,
