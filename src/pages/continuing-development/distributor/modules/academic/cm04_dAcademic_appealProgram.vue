<template>
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">Academic : Appeal Program</h4>
    </div>
    <br />
    <va-card>
...
export default {
  mounted() {
    var data = localStorage.getItem("d-academicAppealProgram");
  ...
  methods: {
    ...
    getDocumentById: async function () {
      const response = await servicesModule4.getAcademicRevokeDocumentById(
        this.PROGDATA.MODULE_ID
      );
      //alert(JSON.stringify(response));
      response.forEach((element) => {
    ...
    accept: async function () {
      if (
        ...
        try {
          const response = await servicesModule4.appealAcademicProgram(data);
          this.$router.go(-1);
    ...
    appeal: async function () {
      if (confirm("Are you sure you want to submit appeal?")) {
        ...
        //notification
        data.append("NOTI_LOCATION", "d-appealAcademicList");
        var remarkEntry ="(CPD-ACADEMIC-APPEAL)New submission pending approval for " + res.DIST_NAME;
    ...
    getAllParticipant: async function () {
      // const response = await servicesModule4.getQualificationParticipantByID(
      //   this.PROGDATA.MODULE_ID
      // );
       const res = await service07Module4.getAcademicParticipantList(this.PROGDATA.MODULE_ID);
    ...
    viewParticipant: async function () {
      console.log("Academic=",this.participantList);
      // localStorage.setItem(
      //   "route_name_viewParticipant",
      //   JSON.stringify(this.participantList)
      // );
      let route = "d-academicViewDetailParticipant";
       localStorage.setItem(route, JSON.stringify(this.participantList));
      localStorage.setItem("route_name_viewParticipant", route);
      localStorage.setItem(
        "title_viewParticipant"," Academic : List of Participants "
      );
       //console.log("dddddddddd=",route);
      this.$router.push({
