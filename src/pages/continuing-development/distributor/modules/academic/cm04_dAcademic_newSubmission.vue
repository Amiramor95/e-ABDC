...
export default {
  ...
  methods: {
    ...
    async uploadExcel() {
      ...
      var response = await this.$refs.vueFileAgent.upload(
        "/api/module4/qualification_module/upload?CREATE_BY=" +
        this.USER.user_id +
        "&COMPANY_ID=" +
    ...
    async saveAsDraft(data) {
      ...
      await service07Module4.academicSubmit(data).then((res) => {
        if(res.status !== 400){
          ...
          this.$router.push("/distributor/d-academicModuleList")
        }
        this.submitLoading = false
      });
    },
  ...
  data() {
    return {
      submitLoading: false,
      title: "Academic/Professional Qualification : New Submission",
      USER: null,
      USERID: null,
      COMPANYID: null,
      recordCount: 0,
