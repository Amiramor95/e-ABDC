  ...
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">
        Appeal Academic/Professional Qualification : List of Distributor
      </h4>
    </div>
...
export default {
  ...
  methods: {
    ...
    getRecordByDist: async function () {
      ...
      const response = await servicesModule4.getAcademicByDist(
        2,
        4
      );
    ...
    async viewRecord(data) {
      localStorage.setItem("d-appealAcademicListProgramList", JSON.stringify(data));
      this.$router.push({
    ...
    RecordFilteredData() {
      ...
      return this.RecordList.filter((item) => {
        return this.term
          ...
          .every(
            (v) =>
              item.DIST_CODE.toString().includes(v)||
