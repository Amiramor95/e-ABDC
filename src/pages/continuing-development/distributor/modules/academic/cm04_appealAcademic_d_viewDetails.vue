  ...
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">
        Academic : Review Appeal Program
      </h4>
    </div>
    <br />
    <va-card>
...
<script>
  ...
  export default {
    mounted() {
      var data = localStorage.getItem("d-appealAcademicViewDetails");
    ...
    methods: {
      ...
      getAppealDocumentById: async function () {
        const response = await servicesModule4.getAcademicAppealDocumentById(
      ...
      saveAsDraft: async function () {
        if (confirm("Are you sure you want to save as draft?")) {
          ...
          data.append("QUALIFICATION_APPEAL_APPR_ID", this.PROGDATA.QUALIFICATION_APPEAL_APPR_ID);
          ...
          try {
            const response = await servicesModule4.updateModuleAppealApproval(data);
      ...
      revokeCpd: async function () {
        if (confirm("Are you sure to revoke this program?")) {
          ...
          //notification
          data.append("NOTI_LOCATION", "d-appealAcademicListProgramList");
          var remarkEntry = "(CPD-ACADEMIC)Your appeal submission has been declined by FIMM";
          ...
          try {
            const response = await servicesModule4.updateAcademicModuleAppealApproval(data);
            this.$router.push({
      ...
      approve: async function () {
        if (confirm("Are you sure you want to approve this program?")) {
          ...
          //notification
          data.append("NOTI_LOCATION", "d-appealAcademicListProgramList");
          var remarkEntry = "(CPD-ACADEMIC)This program has been approved by FIMM";
          data.append("NOTI_REMARK", remarkEntry);
          data.append("DIST_CREATE_BY", this.PROGDATA.DIST_CREATE_BY);
          data.append("DISTRIBUTOR_ID", this.PROGDATA.COMPANY_ID);
      ...
      viewParticipant: async function () {
        localStorage.setItem("approval-academicView-participant",this.PROGDATA.MODULE_ID);
        this.$router.push({
