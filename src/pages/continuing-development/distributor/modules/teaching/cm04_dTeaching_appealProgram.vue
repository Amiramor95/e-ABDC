<template>
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">Teaching : Appeal Program</h4>
    </div>
    <br />
    <va-card>
...
export default {
  mounted() {
    var data = localStorage.getItem("d-teachingAppealProgram");
  ...
  methods: {
    ...
    accept: async function () {
      if (
        ...
        try {
          const response = await servicesModule4.appealTeachingProgram(data);
          this.$router.go(-1);
    ...
    appeal: async function () {
      if (confirm("Are you sure you want to submit appeal?")) {
        ...
        //notification
        data.append("NOTI_LOCATION", "d-appealTeachingList");
    ...
    getAllParticipant: async function () {
      const response = await servicesModule4.getTeachingParticipantByID(
        this.PROGDATA.MODULE_ID
      );
    ...
    viewParticipant: async function () {
      ...
      localStorage.setItem(
        "title_viewParticipant"," Teaching : List of Participants "
      );
      this.$router.push({
