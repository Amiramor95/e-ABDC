...
export default {
  ...
  methods: {
    ...
    viewFundLodge() {
      this.$router.push({ name: "fundLodgementSubmit" });
    },
    handleClick(newTab) {
      this.currentTab = newTab;
    }, // end tab
    async submitLodge() {
      if (confirm("Are you sure to request lodge on the selected fund?")) {
        ...
        try {
          const response = await servicesModule5.submitHoldLodge(data);
    ...
    async removeParticipant(data) {
      if (
        confirm(
          "Are you sure to remove the request lodge on the selected fund?"
        )
      ) {
        const user = localStorage.getItem("user");
        console.log(JSON.stringify(this.user));
        ...
        data.append("FUND_LODGE_LIST", JSON.stringify(this.selected));
        try {
          const response = await servicesModule5.submitRemoveHoldLodge(data);
          alert("Fund selected has been removed to be lodge");
          this.getAllFundLodgement();
        } catch (e) {
          console.log(e);
        }
    ...
    getAllFundLodgement: async function () {
      const user = localStorage.getItem("user");
      const response = await servicesModule5.fund_lodgeHold(
