...
export default {
  ...
  methods: {
    ...
    getAllFundProfile: async function () {
      ...
      const response = await servicesModule5.getFundProfileForManageFM(
        USER_DIST_ID
      );
      this.fundprofileRecordList = response;
    ...
    fundProfileFilteredData() {
      ...
      return this.fundprofileRecordList.filter((item) => {
        return this.term
          ...
          .every(
            (v) =>
              item.FUND_CODE_FIMM.includes(v) || item.FUND_NAME.includes(v) || item.FUNDSHORTNAME(v)
          );
      });
    },
  },
