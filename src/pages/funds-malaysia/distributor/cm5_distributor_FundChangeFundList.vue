...
export default {
  ...
  methods: {
    ...
    getAllFundProfile: async function () {
      ...
      const response = await servicesModule5.FPisActive(USER_DIST_ID);
      this.fundprofileRecordList = response;
    ...
    fundProfileFilteredData() {
      ...
      return this.fundprofileRecordList.filter((item) => {
        return this.term
          ...
          .toUpperCase()
          .split(" ")
          .every((v) => item.FUND_NAME.includes(v) || item.UMBRELLA_FUND_NAME.includes(v) || item.FUND_CODE_FIMM.includes(v) || item.FUND_CODE_MEMBER.includes(v));
      });
    },
  },
