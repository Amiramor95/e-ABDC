...
export default {
  ...
  methods: {
    ...
    back() {
      // this.$router.push("/distributor/fundDisLodgementMainList");
      this.$router.push("fundDisLodgementMainList");
    }, // back button

    // getAllRecord
    getAllFundProfile: async function () {
      // const data = this.$route.params.FUNDHOUSE.FMS_UMBRELLA_FUND_ID;
      // const response = await servicesModule5.getDistributorBasedOnFundHouseForDislodge(
      //   data
      // );

      const response =
        await servicesModule5.getDistributorBasedOnFundHouseSubmit(
          this.selectDistType
        );
      this.fundprofileRecordList = response;
