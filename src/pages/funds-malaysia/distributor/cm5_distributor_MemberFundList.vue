  ...
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">
        List of Fund Management Application
      </h4>
      <br />
      <div class="col float-right">
        <div class="row float-right">
          ...
          <span
            v-b-tooltip.hover
            title="Create New Fund Management Application"
          >
            <h5>
              <b-badge
                v-on:click="create"
...
export default {
  ...
  methods: {
    ...
    getAllFundProfile: async function () {
      ...
      const response = await servicesModule5.getFundProfileForManage(
        USER_DIST_ID
      );
      this.fundprofileRecordList = response;
    ...
    async viewRecord(data) {
      localStorage.setItem("FUNDPROFILE", JSON.stringify(data));
      if (data.TYPE == 1) {
        console.log("type 1");
        if (data.APPL_STATUS == "DRAFT") {
          this.$router.push({
            name: "fundManagementEdit",
            params: { FUNDPROFILE: data },
          });
        } else if (data.APPL_STATUS == "RETURNED") {
          this.$router.push({
            name: "fundManagementEdit",
            params: { FUNDPROFILE: data },
          });
  ...
  computed: {
    fundProfile() {
      return [
        ...
        // {
        //   name: "FUND_PREVIOUS_NAME",
        //   title: "FORMERLY KNOWN AS",
        //   sortField: "FUND_PREVIOUS_NAME",
        // },
        {
          name: "FUNDSHORTNAME",
          title: "FUND SHORT NAME",
          sortField: "FUNDSHORTNAME",
        },
        {
          name: "FUND_LAUNCH_DATE",
          title: "LAUNCH DATE",
          sortField: "FUND_LAUNCH_DATE",
        },
        {
    ...
    fundProfileFilteredData() {
      ...
      return this.fundprofileRecordList.filter((item) => {
        return this.term
          ...
          .toUpperCase()
          .split(" ")
          .every((v) => item.FUND_NAME.includes(v) || item.FUNDSHORTNAME.includes(v) || item.FUND_CODE_FIMM.includes(v) || item.FUND_CODE_MEMBER.includes(v));
      });
    },
  },
