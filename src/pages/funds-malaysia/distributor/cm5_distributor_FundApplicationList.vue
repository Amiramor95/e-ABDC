<template>
  <va-inner-loading :loading="statusLoad">
    <va-card>
      <div class="row">
        <h4 slot="header" class="card-title">List of Fund</h4>
        <br />
      ...
      <va-card class="mt-5 col-12">
        ...
        >
          ...
          <template slot="status" slot-scope="props">
            <a v-if="props.rowData.IS_SUBMIT == '1'">
              DRAFT
            </a>
            <a v-if="props.rowData.TS_PARAM == 'APPROVED' && props.rowData.IS_SUBMIT == '2'">
              APPROVED
            </a>
            <a v-if="props.rowData.TS_PARAM == 'PENDING' && props.rowData.IS_SUBMIT == '2'">
              PENDING
            </a>
            <a v-if="props.rowData.TS_PARAM == 'NEW ENTRY' && props.rowData.IS_SUBMIT == '2'">
              NEW ENTRY
            </a>
            <a v-if="props.rowData.TS_PARAM == 'ACTIVE' && props.rowData.IS_SUBMIT == '2'">
              ACTIVE
            </a>
            <a v-if="props.rowData.TS_PARAM == 'REJECTED' && props.rowData.IS_SUBMIT == '2'">
              REJECTED
            </a>
            <a v-if="props.rowData.TS_PARAM == 'RETURNED' && props.rowData.IS_SUBMIT == '2'">
              RETURNED
            </a>
          </template>
...
export default {
  mounted() {
    localStorage.removeItem("fundApplication");
  ...
  methods: {
    ...
    getAllFundProfile: async function() {
      ...
      const response = await servicesModule5.getFundProfileActive(USER_DIST_ID);
      this.fundprofileRecordList = response;
    ...
    fundProfile() {
      return [
        ...
        },
        {
          ...
          title: "LAUNCH PRICE PER UNIT",
          sortField: "FUND_PRICE_PERUNIT"
        },
        // {
        //   name: "APPL_STATUS",
        //   title: "FUND STATUS",
        //   sortField: "APPL_STATUS"
        // },
        {
          name: "FUND_STATUS_FUND",
          title: "FUND STATUS",
          sortField: "FUND_STATUS_FUND"
        },
        {
          name: "__slot:status",
    ...
    fundProfileFilteredData() {
      ...
      return this.fundprofileRecordList.filter(item => {
        return this.term
          ...
          .toUpperCase()
          .split(" ")
          .every(v => item.FUND_NAME.includes(v) || item.FUND_STATUS_FUND.includes(v) || item.FUND_PRICE_PERUNIT.includes(v) || item.SET_PARAM.includes(v));
      });
    }
  },
