...
export default {
  ...
  methods: {
    ...
    // getAllRecord
    getAllFundApplication: async function() {
      const user = localStorage.getItem("user");
      var USER_DIST_ID = JSON.parse(user).USER_DIST_ID;
      const response = await servicesModule5.getAllFundApplication(
        USER_DIST_ID
      );
      this.fundprofileRecordList = response;
    ...
    async viewRecord(data) {
      localStorage.setItem("fund_ApplicationFMReview", JSON.stringify(data));
      this.$router.push({
