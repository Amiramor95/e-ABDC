    ...
    <div class="row">
      ...
      <!-- <div class="col float-right">
        ...
        <div class="row float-right">
            <span v-b-tooltip.hover title="Create New Fund">
              <h5>
                <b-badge
                  v-on:click="create"
    ...
    <va-card class="mt-5 col-12">
      ...
      >
        ...
        <template slot="remark" slot-scope="rem1">
          <span v-if="rem1.rowData.Status=='APPROVED' || rem1.rowData.Status=='RETURNED'" v-b-tooltip.hover title="view" v-on:click="viewmodel(rem1.rowData)" class="mr-2" >
              <i class="fa fa-clone"></i
            ></span>
             <p v-if="rem1.rowData.Status=='DRAFT'|| rem1.rowData.Status=='ACTIVE'"
            >
        ...
        <template slot="actions" slot-scope="props">
          <h5 class="mt-1">
            <span v-if="props.rowData.Status=='DRAFT' || props.rowData.Status=='RETURNED'"
              v-b-tooltip.hover
              title="view"
            ...
            >
              <i class="fa fa-eye"></i
            ></span>
             <span v-if="props.rowData.Status=='APPROVED' || props.rowData.Status=='ACTIVE' || props.rowData.Status=='PENDING'"
              v-b-tooltip.hover
              title="view"
                v-on:click="viewRecord(props.rowData)"
              class="badge badge-primary mr-2"
...
export default {
 ...
 viewmodel(data){
    ...
    getAllFundApplicationfimm: async function() {
      const response = await servicesModule5.nav_list();
      this.fundprofileRecordList = response;
      this.fundprofileRecordCount = this.fundprofileRecordList.length;
      this.statusLoad = false;
    },
    async viewRecord(data) {
      localStorage.setItem("navupdatelist", JSON.stringify(data));
      this.$router.push({
     ...
     create() {
       var obj={};
       obj.Status="DRAFT";
      localStorage.setItem("navupdatelist", JSON.stringify(obj));
      this.$router.push({ name: "navUpdateVerify" });
    },
  },
