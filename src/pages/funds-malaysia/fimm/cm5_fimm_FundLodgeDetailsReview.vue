...
<va-inner-loading :loading="statusLoad">
  <div class="flex xs12 md12 xl12">
    ...
    >
      ...
      <div>
        <br />
        <h4 slot="header" class="card-title">Fund Lodgement Details</h4>
    ...
    <va-card>
                ...
                <table class="table table-responsive table-borderless">
                  <tbody>
                    <tr >
                      ...
                      <td>{{ FUNDPROFILE.SECOND_LEVEL_REMARK }}</td>
                    </tr>
                    <tr v-if="FUNDPROFILE.TS_PARAM_FIRST != 'PENDING'">
                      <td style="width: 15%">
                        <b>FIMM Remark</b>
                      </td>
                      <td style="width: 1%">
      ...
      <br />
      <div v-if="FUNDPROFILE.TS_PARAM_FIRST == 'PENDING'">
        <vue-form-generator
          :model="model"
          :schema="schema"
          :options="formOptions"
  ...
  methods: {
    ...
    returnCase: async function () {
      ...
      if (response) {
        ...
        data.append("FUND_LODG_ID", this.FUNDPROFILE.FUND_LODG_ID);
        ...
        try {
          if (confirm("Are you sure to Return this fund lodgement ?")) {
            const response =
              await servicesModule5.approvalFundLodgeFimm(data);
            alert("Fund Lodgement Submission Returned");
            this.back();
          }
        } catch (error) {
          console.log(error);
    ...
    async rejectCase() {
      if (confirm("Are you sure to Reject this fund lodgement ?")) {
        ...
        data.append("BULK_APPROVAL", 2);
        try {
          const response = await servicesModule5.approvalFundLodgeFimm(data);
