<template>
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">NAV Management : Submission for approval</h4>
    </div>
    <br />
...
import * as services01Module5 from "../../../../app/module5/services01";
  ...
  export default {
    mounted() {
      var data = localStorage.getItem("navManagementApproval");
    ...
    methods: {
      ...
      const response = await services01Module5.getFimmAuditLog(
        this.WDATA.NAV_TEMP_MASTER_ID
      );
    ...
    if (confirm("Are you sure you want to approve this submission?")) {
      ...
      data.append("NAV_TEMP_FIMM_APPROVAL_ID", this.WDATA.NAV_TEMP_FIMM_APPROVAL_ID);
      data.append("NAV_TEMP_MASTER_ID", this.WDATA.NAV_TEMP_MASTER_ID);
      data.append("DISTRIBUTOR_ID", this.WDATA.DISTRIBUTOR_ID);


      //notification
      data.append("DIST_NOTI_LOCATION", "navManagementList");
      var remarkEntry = "(NAV MANAGEMENT)Your NAV Submission has been Approved by Fimm";
      data.append("DIST_NOTI_REMARK", remarkEntry);



      try {
        const response = await services01Module5.updateFimmApproval(data);
        this.$router.push("/fimm/navManagementApprovalList");
      } catch (error) {
        console.log(error);
      }
    }
    ...
    if (confirm("Are you sure you want to reject this submission?")) {
      ...
      //notification
      data.append("DIST_NOTI_LOCATION", "navManagementList");
      var remarkEntry = "(NAV MANAGEMENT)Your NAV Submission has been rejected by FIMM ";
      data.append("DIST_NOTI_REMARK", remarkEntry);



      try {
        const response = await services01Module5.updateFimmVerify(data);
