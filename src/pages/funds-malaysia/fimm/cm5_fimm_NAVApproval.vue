<template>
  <va-card>
    <div class="row">
      <h4 slot="header" class="card-title">NAV Approval</h4>
      <br />
    </div>
...
export default {
  ...
  methods: {
    ...
    getAllFundProfile: async function () {
      ...
      var NAV_TEMP_BULK_ID = JSON.parse(NAVDATA).NAV_TEMP_BULK_ID;
    ...
    async onSubmit() {
      try {
        if (confirm("Are sure to Approve this NAV daily update ?")) {
          ...
          data.append("APPR_REMARK_ID", this.model.REMARKS_COMMENTS);
          data.append("APPR_PUBLISH_STATUS", "1");
          data.append("ID_TS_ID", "3");
          data.append("TS_ID", "15");
        //notification
        data.append("NOTI_LOCATION", "cm5_fimm_NAVManagementHODIDCompanyList");

        data.append("NOTI_REMARK", "(FIMM) NEW NAV Management Submission");
        ...
        data.append("APPR_LIST", JSON.stringify(approvalLevel));
          const response = await servicesModule5.submitTempNavApprovalID(data);
          alert("NAV Daily update has been approve and send to FiMM HOD");444n
          this.$router.push("navManagementIdApprovalList");
        }
      } catch (e) {
        console.log(e);
      }
    ...
    async reject() {
      try {
        if (confirm("Are sure to reject this NAV update ?")) {
          const data = new FormData();
          const user = localStorage.getItem("user");
          const NAVDATA = localStorage.getItem("NAVDATA");
          data.append("NAV_TEMP_BULK_ID", JSON.parse(NAVDATA).NAV_TEMP_BULK_ID);
