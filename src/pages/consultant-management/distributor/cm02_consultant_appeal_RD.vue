          ...
          <template v-if="!currentDistributor">
            <va-card>
              <h4 style="margin-top:0px; margin-bottom:20px">
                List of New Appeal Application (RD)
              </h4>
          ...
          <template v-else>
            ...
            <template v-else>
              ...
              <va-card>
                ...
                <div v-if="currentConsultant.APPEAL_FIMM_STATUS_ID==15">
                  <vue-form-generator
                    :model="model2"
                    :schema="tabSchema5"
                    :options="formOptions"
                ...
                <div class="float-right" v-if="currentConsultant.APPEAL_FIMM_STATUS_ID==15">
                  <button
                  ...
                  <button
                    @click="approveAppeal(3, 'Are you sure to approve?')"
                    type="button"
...
export default {
    ...
    return {
      ...
      fields: {
        distributor: [
          ...
          { name: 'APPEAL_TYPE', title: 'APPEAL TYPE' },
        ...
        consultant: [
          ...
          { name: 'TYPE_SCHEMAS', title: 'APPLICATION TYPE' },
          { name: 'APPEAL_TYPE', title: 'APPEAL TYPE' },
          { name: 'TS_PARAM', title: 'STATUS' },
          { name: '__slot:ACTION', title: 'ACTION' },
        ],
            ...
            fields: [
              {
                ...
                rows: 5,
                max: 500,
                label: 'Remark by Distributor Admin',
                hint: 'maximum of 500 characters',
            ...
            fields: [
              {
                ...
                rows: 10,
                max: 500,
                label: 'Remark by Distributor Manager',
                hint: 'maximum of 500 characters',
                model: 'DIST_MANAGER_REMARK',
                placeholder: 'Enter Remarks or Comments',
                // validator: 'number',
                styleClasses: 'col-md-12',
                disabled: true,
  ...
  methods: {
    ...
    getDistributorDetails: async function() {
      ...
      this.distributorsAPI = await services04module2.getDistributorAppealDetails();
    ...
    async getConsultantRecord() {
      const response = await services04module2.getAppealForRDApproval(
        this.currentDistributor.DISTRIBUTOR_ID,
        this.currentDistributor.APPEAL_TYPE_ID
      );
    ...
    approveAppeal: async function(status, confirm_text) {
       if(confirm(confirm_text)){
        try {
          ...
          data.append("APPEAL_DATE", this.currentConsultant.APPEAL_DATE);
          data.append("DISTRIBUTOR_ID", this.currentDistributor.DISTRIBUTOR_ID);
          data.append("FIMM_REMARK", this.model2.FIMM_REMARK ? this.model2.FIMM_REMARK : '');
          data.append("NOTI_APPROVE_REMARK", '(CONSULTANT APPEAL) Approved by RD');
          data.append("NOTI_RETURN_REMARK", '(CONSULTANT APPEAL) Returned by RD');
          data.append("NOTI_REJECT_REMARK", '(CONSULTANT APPEAL) Rejected by RD');
          
          data.append("APPR_GROUP_ID_ADMIN", "3");  //Approval Group ID for ADMIN OF DISTRIBUTOR in am/DISTRIBUTOR_MANAGE_GROUP
          data.append("APPR_GROUP_ID_MANAGER", "1");  //Approval Group ID for Notification to MANAGER OF DISTRIBUTOR in am/DISTRIBUTOR_APPROVAL_LEVEL
          data.append("APPR_GROUP_ID_CONST", "1");  //Group ID for New Consultant in am/CONSULTANT_MANAGE_GROUP
          data.append("APPR_GROUP_ID_FIMM", "4");  //Group ID for FIMM(RD) Registration Department in am/MANAGE_GROUP
          ...
          data.append('APPEAL_TYPE', this.currentConsultant.APPEAL_TYPE_ID);

          if(status == 3){ //APPROVED
            data.append("TS_ID", "3"); //Prefer admin_management DB -> TASK_STATUS for TS_ID
          }else if(status == 8){ //RETURNED to Distributor Manager
            data.append("TS_ID", "8");
          }else if(status == 5){ //REJECTED - NOTIFY TO CONSULTANT
            data.append("TS_ID", "5");
          }

          if (status!=3 && this.model2.FIMM_REMARK== '') {
            Vue.$toast.open({
            ...
            this.show_overlay = true;   
            const response = await services04module2.appealRDReview(data);
            if(response){
              this.show_overlay = false;
              Vue.$toast.open({
