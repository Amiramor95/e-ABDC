          ...
          <b-overlay :show="show_overlay" rounded="sm">
              ...
              <va-card>
                ...
                >
                  <template slot="checkbox" slot-scope="props">
                    <b-form-checkbox v-if="props.rowData.RENEWAL_STATUS==1"
                ...
                <vue-form-generator
                  v-if="this.REVIEWER_STATUS==1 || this.REVIEWER_STATUS==15 || this.FIMM_STATUS==8 || this.FIMM_STATUS==5"
                  :model="model"
                  :schema="tabSchema2"
                  :options="formOptions"
                  ref="tabForm2"
                ...
                <div class="float-right" v-if="this.REVIEWER_STATUS==1 || this.REVIEWER_STATUS==15 || this.FIMM_STATUS==8 || this.FIMM_STATUS==5" >
            ...
            <template v-else>
              ...
              <va-card>
                ...
                <div v-if="this.REVIEWER_STATUS==1 || this.REVIEWER_STATUS==15 || this.FIMM_STATUS==8 || this.FIMM_STATUS==5">
                  <vue-form-generator
                    :model="model"
                    :schema="tabSchema2"
                    :options="formOptions"
...
export default {
  ...
  async mounted() {
    ...
    this.REVIEWER_STATUS = JSON.parse(localStorage.getItem('REVIEWER_TS_ID'));
    this.SUBMISSION_DATE = localStorage.getItem('SUBMISSION_DATE');
    this.FIMM_STATUS = localStorage.getItem('FIMM_STATUS');
    ...
    await this.getConsultantRecord(DIST_INFO.USER_DIST_ID, this.REVIEWER_STATUS, this.SUBMISSION_DATE);
  ...
  computed: {
    ...
    consultantFields() {
      if (this.REVIEWER_STATUS==1 || this.REVIEWER_STATUS==15 || this.FIMM_STATUS==8 || this.FIMM_STATUS==5) {
        return [
          {
  ...
  methods: {
    previous(){
      this.$router.push('/distributor/consultant-non-renewal-Reviewer');
    },
    ...
    async getConsultantRecord(distId, status, submissionDate) {
        ...
        const response = await services02module2.getRenewalConsultantReviewer(distId, JSON.stringify([status]), submissionDate);
        this.consultantsAPI = response;
        response.forEach(element => {
          if(status!=15){
            ...
            if(element.REMARK !=null || element.REMARK != ''){
              this.model.RENEWAL_REMARK = element.REMARK;
            }
          }
        });
      ...
      if(this.model.CONSULTANT_RENEWAL_ID.length>0){
        if (confirm(confirm_text)) {
          ...
          if(status==3){ //Approved
            const approvalLevel = await services06Module0.getApprovalLevelDistributor(39, 1); // (processflowID, approval_level_index)
          ...
          if(status==3){ //Approved
            data.append("FIMM_STATUS", 15);
            data.append("NOTI_LOCATION", "consultant-renewal-Manager");
            data.append(
              "NOTI_REMARK",
            ...
            data.append(
              "NOTI_REMARK",
              "(CONSULTANT NON-RENEWAL) has been rejected by Reviewer "
            );
          }else if(status==8){ //Return
            data.append("NOTI_LOCATION", "consultant-renewal-Manager");
            data.append(
              "NOTI_REMARK",
              "(CONSULTANT NON-RENEWAL) Renewal has been returned by Reviewer"
            );
          }

          const response = await services02module2.postConsultantRenewalReviewer(data);
          this.show_overlay = false;
          this.$router.push('/distributor/consultant-non-renewal-Reviewer')
        }
      }else{
