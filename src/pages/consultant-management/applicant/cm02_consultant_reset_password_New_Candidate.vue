...
export default {
  ...
  methods: {
    validatePassword(value) {
      ...
      console.log(expression6.test(value));
      if (!expression6.test(value)) {
        this.passwordStrength = '';
        return [
          'must contain at least one uppercase letter, one lower case letter, one number and one character with minimum length of 8',
        ];
      }
    },
    isEqualTo() {
      if (this.model.PASSWORD !== this.model.CONFIRM_PASSWORD) return ['Password do not match'];
      if (this.model.CONFIRM_PASSWORD === '') return ['Confirm Password is required'];
    },
