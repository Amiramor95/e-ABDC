            ...
            <template>
              ...
              <va-card v-else>
                <template>
                    ...
                    <br />
                    <vue-form-generator
                      v-if="this.APPROVAL_STATUS==1 || this.APPROVAL_STATUS==15"
                      :model="model"
                      :schema="tabSchema2"
                      :options="formOptions"
                      ref="singleForm"
                    ...
                    <div class="float-right" v-if="this.APPROVAL_STATUS==1 || this.APPROVAL_STATUS==15" >
...
<script>
  ...
  export default {
    ...
    async mounted() {
      ...
      this.model.APPR_REMARK=this.currentConsultant.APPROVAL_REMARK;
    ...
    computed: {
      ...
      RecordFilteredData() {
        const search = this.search.records.toLowerCase();
        if (this.search && this.RecordList) {
          return this.RecordList.filter(el => {
            return Object.keys(el).some(key => `${el[key]}`.toLowerCase().match(search));
          });
        }
    ...
    methods: {
      getGMAuditLog: async function () {
        const response = await services02module2.getGMAuditLogByID(
          this.currentConsultant.TERMINATION_ID
        );
