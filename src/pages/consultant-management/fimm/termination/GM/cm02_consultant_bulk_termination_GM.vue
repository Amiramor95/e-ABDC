...
<script>
  ...
  export default {
    ...
    async mounted() {
      ...
      // this.APPROVAL_STATUS.push(JSON.parse(localStorage.getItem('APPROVAL_STATUS')));
    ...
    methods: {
      ...
      async getDraftedData(DIST_ID, SUBMISSION_ID){
        const response=await services02module2.getDraftTerminationApproval(DIST_ID, SUBMISSION_ID);
        this.consultantsAPI = response;
        response.forEach(element => {
          this.model.APPROVAL_REMARK=element.APPROVAL_REMARK;
      ...
      back() {
        this.$router.push('/fimm/consultant-termination-statusList-GM')
      }, // back button
      ...
      async getConsultantRecord(distId, submissionId) {
        ...
        if(this.APPROVAL_STATUS==15){
          const response = await services02module2.getAllConsultantList(distId,JSON.stringify([15]),submissionId);
          this.consultantsAPI = response;
          this.isReady = true;
        }
        else{
          const response = await services02module2.getAllConsultantList(distId,JSON.stringify([this.APPROVAL_STATUS]),submissionId);
          this.consultantsAPI = response;
          this.isReady = true;
        }
      },
      ...
      async viewCurrentConsultant(props) {
        try {
          ...
          this.$router.push('/fimm/consultant-termination-details-GM');
        } catch (error) {
          console.log(error);
        }
      },
      ...
      toggleCheckboxAll(isChecked){
        ...
        toggleCheckbox(payload){
          ...
          if(fieldName.TERMINATION_STATUS==1){
            //alert('You have already drafted this consultant!')
          }else{
            if(!isChecked){
      ...
      saveAsDraft: async function() {
        if(this.model.TERMINATION_ID.length>0){
          if (confirm("Are you sure you want to save as draft?")) {
            ...
            data.append("APPR_REMARK",this.model.APPROVAL_REMARK ? this.model.APPROVAL_REMARK : '')
      ...
      submitTerminationForm: async function() {
        if(this.model.TERMINATION_ID.length>0){
          if (confirm("Are you sure to submit?")) {
            ...
            const approvalLevel = await services06Module0.getApprovalLevel(9, 3);
            data.append("APPR_LIST", JSON.stringify(approvalLevel));

            //dashboard notification
            data.append("NOTI_LOCATION", "consultant-termination-CEO");
            data.append(
              "NOTI_REMARK",
              "(CONSULTANT-TERMINATION) New entry pending for CEO Approval"
            );
