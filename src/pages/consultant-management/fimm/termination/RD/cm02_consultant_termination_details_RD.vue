...
export default {
  ...
  async mounted() {
    ...
    this.model.CONSULTANTS.push(
      {
        ...
        CONSULTANT_TYPE_ID: this.currentConsultant.CONSULTANT_TYPE_ID,
      }
    );
    ...
    // if(!this.isEditable){
      ...
      //if(this.currentConsultant.TERMINATION_TYPE_OTHER_SPECIFY !=null && this.currentConsultant.TERMINATION_TYPE_OTHER_SPECIFY !="" && this.currentConsultant.TERMINATION_TYPE_ID==10){
  ...
  methods: {
    ...
    saveAsDraft: async function() {
      ...
      if (confirm("Are you sure to save as draft?")) {
        ...
        data.append("TERMINATION_STATUS", 1); //Save as Draft
    ...
    submitTerminationForm: async function() {
      ...
      // if(isSingleValidated) {
        if (confirm("Are you sure to terminate?")) {
          this.show_overlay = true;
          const data = new FormData();
          data.append("TERMINATION_STATUS", 15); // Submit Pending Request
          data.append("CONSULTANTS", JSON.stringify(this.model.CONSULTANTS));
          data.append("CREATE_BY", this.model.CREATE_BY);
          const user = localStorage.getItem("user");
          data.append("APPR_GROUP_ID", JSON.parse(user).USER_GROUP_ID);
