              ...
              >
                ...
                <template slot="ACTION" slot-scope="props">
                  <h4 style="margin:0; font-size:20px">
                      <span v-b-tooltip.hover title="Edit" class="badge badge-primary mr-2" @click="viewListPage(props)"  v-if="props.rowData.APPROVAL_STATUS==15 || props.rowData.APPROVAL_STATUS==1">
...
export default {
  async created () {
    ...
    this.consultantsAPI=await services02module2.getAllHodConsultantTermination(DIST_INFO.DISTRIBUTOR_ID);
    this.isReady=true;
  },
  async mounted() {
  ...
  methods: {
    back() {
      this.$router.push('/fimm/consultant-termination-RD_HOD')
    }, // back button
    ...
    viewListPage(props){
      ...
      this.$router.push('/fimm/consultant-termination-bulk-RD-HOD');
    },

    // editListPage(props){
    //   localStorage.setItem('APPROVAL_STATUS',[1,15]);
    //   this.$router.push('/fimm/consultant-termination-bulk-RD-HOD');
    // },
