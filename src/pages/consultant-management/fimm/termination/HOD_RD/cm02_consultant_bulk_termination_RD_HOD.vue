              ...
              <va-card v-else>
                ...
                <div v-if="APPROVAL_STATUS==1 || APPROVAL_STATUS==3 || APPROVAL_STATUS==5 || APPROVAL_STATUS==7">
                  <va-data-table
                    ...
                    clickable
                  >
                    <template slot="checkbox" slot-scope="props" v-if="APPROVAL_STATUS==1">
                      <b-form-checkbox
                ...
                <template>
                  <vue-form-generator
                    v-show="APPROVAL_STATUS==1 || APPROVAL_STATUS==15"
                    :model="model"
                    :schema="tabSchema2"
                    :options="formOptions"
                    ref="bulkForm"
...
import fundHistoricalNavUserReportVue from '../../../../admin-configuration/reports/FUND/fund-historical-nav-user-report.vue';
...
export default {
    ...
    return {
      ...
      fields: {
        ...
        consultant: [
          ...
          { name: 'APPR_REMARK', title: 'REMARKS/COMMENTS' },
  ...
  methods: {
      async getDraftedData(DIST_ID, SUBMISSION_ID){
        const response=await services02module2.getAllDraftedHODTermination(DIST_ID, SUBMISSION_ID);
        this.consultantsAPI = response;
        response.forEach(element => {
          this.model.CONSULTANT_ID.push(element.CONSULTANT_ID);
          this.model.TERMINATION_DATE=element.TERMINATION_DATE;
          this.model.APPR_REMARK=element.APPR_REMARK;
    ...
    async getConsultantRecord(distId, submissionId) {
      ...
      if(this.APPROVAL_STATUS==15){
        const response = await services02module2.getAllHODTerminationList(distId,JSON.stringify([15]), submissionId);
        this.consultantsAPI = response;
        this.isReady = true;
      }
      else{
        const response = await services02module2.getAllHODTerminationList(distId,JSON.stringify([this.APPROVAL_STATUS]), submissionId);
        this.consultantsAPI = response;
        this.isReady = true;
      }
    },
    ...
    async viewCurrentConsultant(props) {
      try {
        ...
        this.$router.push('/fimm/consultant-termination-details-RD-HOD');
      } catch (error) {
        console.log(error);
      }
    },
    ...
    rejectTermination: async function() {
      if(this.model.TERMINATION_ID.length>0){
        if (confirm("Are you sure to reject?")) {
          ...
          const response = await services02module2.updateTerminateConsultant(data,JSON.stringify(this.model.TERMINATION_ID));
          this.show_overlay = fundHistoricalNavUserReportVue;
          this.$router.push('/fimm/consultant-termination-statusList-RD-HOD')
        }
      }else{
        alert('Please select consultants for action');
