    ...
    <va-card class="mt-5 col-12">
      ...
      >
        ...
        <template slot="action" slot-scope="row">
          <button type="button" @click="remarksModal(row.rowData)" :class="row.rowData.AUM_REMARKS_HOD_RD || FILL_IDS.includes(row.rowData.AUM_ENTRY_ID) ?
      ...
      <div>
        ...
        <div class="float-right">
          <button @click="VerifyFund('draft')" type="button" class="mr-2 btn btn-info btn-fill btn-md">
                  <i class="fa fa-pencil-square-o" /> &nbsp; Save as draft
          </button>
          <button @click="VerifyFund('return_to_dist')" :disabled="!aumEntryIds.length" type="button" class="mr-2 btn btn-warning btn-fill btn-md">
                  <i class="fa fa-reply" /> &nbsp; Return to Distributor
          </button>
          <button @click="VerifyFund('return_to_r_a')" :disabled="!aumEntryIds.length" type="button" class="mr-2 btn btn-warning btn-fill btn-md">
                  <i class="fa fa-reply" /> &nbsp; Return to R & A
          </button>
          <button @click="VerifyFund('complete')" :disabled="aumEntryIds.length !== RecordList.length"  type="button" class="ml-1 btn btn-success btn-fill btn-md">
...
export default {
  ...
  created () {
    this.getAllRecord()
    this.$on('vuetable:checkbox-toggled', this.toggleCheckbox)
    this.$on('row-clicked',this.toggleCheckboxAll)
  ...
  methods: {
    ...
    viewList() {
      this.$router.push("/fimm/AnnualFee-HODRD-AUMVerification-Details");
    },
    ...
    remarksModal (row) {
        ...
        this.remark = row.AUM_REMARKS_HOD_RD
      },
      ...
      saveRemarkModalComment () {
        const requestPayload = {
          ...
          REMARK: this.remark,
          REMARK_TYPE: 'HOD_RD_REMARKS',
        }

        servicesModule7.postRemarksForDistributorAumEntry(requestPayload).then(res => {
          if (res.status) {
            this.$toast.success('HOD RD remark successfully save.')
        ...
        this.RecordList[index].AUM_REMARKS_HOD_RD = this.remark
      },
    ...
    back () {
      this.$router.push('/fimm/AnnualFee-HODRD-Verification-list')
    },
     ...
     VerifyFund: async function (mode) {
      if (confirm('Are you sure?')) {
          ...
          formData.append('MODULE_NAME','ANNUAL_FEE_VERIFICATION_HOD_RD')
          const response = await servicesModule7.postAumEntryFimm(formData)
          if (response.status === 200) {
            this.$toast.success('AMSF  successfully submitted.')
          }
        }
    },
    ...
    getAllRecord: async function () {
      const response = await servicesModule7.aumFundEntryFimm(this.$route.params.DIST_ID)
