  ...
  methods: {
    ...
    viewRecord(props){
       this.$router.push({
          ...
          params:{DIST_ID:props.DISTRIBUTOR_ID}
       })
    },
    ...
    async btnAction (param) {
      ...
      if(confirm('Do you want to perform ' + prepareTxt))
      {
        const data = {
          BTN_MODE : param,
          MODULE_NAME: 'ANNUAL_FEE_VERIFICATION_FIN',
          FUND_SUMMARY_IDS:this.FUND_SUMMARY_IDS
        }
        const res =  await servicesModule7.postfinVerification(data)
        
        if(res.status === 200){
          this.$toast.success('Annual Fee Verification FIN successfully updated.')
          this.getAllRecord()
        }
      }
    ...
    getAllRecord: async function () {
      const response = await servicesModule7.getAnnualFeeFinance()
