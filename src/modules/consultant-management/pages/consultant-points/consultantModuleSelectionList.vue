    ...
    <va-card>
      <table class="table table-striped">
        ...
        <tr>
          ...
          <td style="width: 1%">:</td>
          <td>{{ PROGDATA.CONSULTANT_FIMM_NO != null ? PROGDATA.CONSULTANT_FIMM_NO : '-' }}</td>
        </tr>
        <tr>
          <td style="width: 20%;font-weight: bold;">CUMULATIVE POINT</td>
          <td style="width: 1%">:</td>
          <td style="font-weight: bold">{{this.CUMULATIVE_POINT}}</td>
        </tr>
        <tr>
...
<script>
  ...
  export default {
    mounted () {
      ...
      this.PROGDATA=JSON.parse(data).consultant;
      this.getCumulativePoint(this.PROGDATA.CONSULTANT_ID);
      this.getFPModules();
    ...
    methods: {
      ...
      async getCumulativePoint(CONSULTANT_ID){
        try {
          this.CUMULATIVE_POINT = await services02module2.getCumulativePoint(CONSULTANT_ID);
        } catch (error) {
          console.log(error);
        }
      },
      ...
      async getDistributorDetails() {
        try {
          const res_distributor = await services02module2.getConsultantLicenseWithDist(this.PROGDATA.CONSULTANT_ID);
      ...
      getFilteredData: async function (value) {
        this.isReady=true;
        const response = await services02module2.getConsultantCPDDetails(value,this.PROGDATA.CONSULTANT_ID,)
