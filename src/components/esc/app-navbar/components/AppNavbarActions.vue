...
import NotificationDropdown from '../../../notification/index_esc'
...
export default {
  ...
  mounted () {
    ...
    if (JSON.parse(user).name.length > 14) { this.userName = JSON.parse(user).name + '...' } else { this.userName = JSON.parse(user).name }
  },
  props: {
    isTopBar: {
      type: Boolean,
