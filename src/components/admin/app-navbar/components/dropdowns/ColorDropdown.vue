...
export default {
  ...
  computed: {
    ...
    computedActiveColor: function() {
      console.log("Colorlist=",this.activeColorList);
  ...
  methods: {
     ...
     getActiveColorSetting: async function () {
        ...
        const response = await servicesModule0.getActiveColorCode(user_id,user_type);
    ...
    async setTheme(data){
      ...
      var user_id = JSON.parse(user).user_id;
      var user_type = JSON.parse(user).user_type;
     // console.log("ColorUser=",user_id);
     // console.log("Color=",data);
      const response = await servicesModule0.getColourById(data,user_id,user_type);
      //localStorage.setItem('user_colour',JSON.stringify(response));
